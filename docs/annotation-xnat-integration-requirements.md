...
Functional requirements for moving RTSTRUCT and DICOM SEG containers between the XNAT Workstation app and an XNAT server (and, by extension, other backends with similar semantics). Covers loading, saving, versioning, conflict detection, and the lifecycle around them.
...
### C6. Save during edit
*To fill in: how the transport implements the multi-viewport E2 guarantee (edits during save are not lost) — save-then-amend vs queue-next-save (the multi-viewport doc leaves this decision to the design phase).*
...
- **Trigger model for autosave** (C1): per-edit, debounced, manual only, or hybrid? Multi-viewport doc leaves this here.
- **Save-during-edit semantics** (C6): save-then-amend vs queue-next-save.
