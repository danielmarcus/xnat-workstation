...
| ~~"Active only" toggle~~ | — | ❌ removed per review (dimming + pill convey it) — **superseded 2026-09-16**: the list is viewport-scoped, so focus *is* the filter. See §2 note below. |
...
- **Filter / search and "Active only" removed.** Annotation counts per session are small enough that filtering isn't worth the chrome; active-viewport state is already shown by row dimming + the cross-panel pill. Sort (Created / A–Z / Size) is kept. Specs: requirements D7.7, CLAUDE.md UI arch.
  - **Superseded 2026-09-16 — §2 re-approved with viewport scoping.** The reasoning above rested on the dimming and the pill conveying active-viewport state. Neither was ever wired: `crossPanelCount` and `MemberRow.eligibility` were declared down to the row components and never supplied by any caller, so no build ever rendered them. What shipped was an undifferentiated list spanning every scan on screen. The list is now scoped to the focused viewport; the pill and the dimming are wired and kept, the pill now meaning "also on N other viewports". Still **no toggle** — that part of the decision stands. See [`viewport-scoped-annotations-proposal.md`](viewport-scoped-annotations-proposal.md).
