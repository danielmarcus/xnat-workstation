...
1. **Container** — one entry per loaded annotation object. Container types:
   ...
   - **POI list** (Points of Interest) — members are individual point annotations (isocenters, fiducials, localization points). Stored as DICOM RTSTRUCT with `RTROIInterpretedType=MARKER` or as a POI-specific format. **POI containers are out of scope for v1 detailed UX requirements**, but the data model and panel must accommodate them as a first-class container type so v2 work does not require schema changes.
...
2. **Member** — one entry per ROI / segment / POI. Members carry the row-level metadata listed in D7.2.
...
- **POI (Points of Interest) detailed UX** — first-class point annotations (isocenter, fiducials, localization). The container type is recognized in the v1 data model and panel hierarchy (D7.1) so future work does not require schema changes; full editing UX is deferred.
...
2. Open same series in two stack panels at different slice indices. Edit a contour on panel A's slice; panel B unaffected (different slice). Scroll panel B to the edited slice; the edit is there.
