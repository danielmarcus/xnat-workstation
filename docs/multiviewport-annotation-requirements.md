...
> **⚠ Superseded in part, 2026-09-16.** The scope rule and the retention rule below were replaced by viewport scoping + prompt-on-leave. The bullets are kept for the record and each superseded one is marked; the current rule is in [`viewport-scoped-annotations-proposal.md`](viewport-scoped-annotations-proposal.md) and summarised in `CLAUDE.md`. The load, navigate and no-phantom-state bullets are unaffected.

- ~~**Scope = the loaded study/session, not a single scan or viewport.**~~ **SUPERSEDED** — the panel is scoped to the **focused viewport**. The rule below assumed the list could stay undifferentiated because dimming and the cross-panel pill marked which rows belonged where; neither was wired in any build, so the list was undifferentiated with nothing to differentiate it. Both are wired now and kept, alongside the scoping. *Original text:* The panel shows **every** annotation container associated with the session currently loaded in the viewport area — across all its scans/series — subject only to FoR-eligibility for *rendering* (A2). A container is never hidden from the *list* because the active viewport happens to show a different series (it dims / shows a cross-series or "not viewable here" marker per D9 / A2d / D7.4, but stays listed).
...
- ~~**Switch to a different session/study — retain unsaved.**~~ **SUPERSEDED** — leaving now **prompts**: Save · Discard · Cancel, fired when a load would leave a container with unsaved edits showing in no viewport at all. Retention did keep unsaved work safe, but it produced containers listed in the panel while rendering nowhere, identified by a scan-id badge that is blank for precisely that work (it is populated only once saved). *Original text:* Loading a scan from a *different* session **re-scopes** the panel to that session (replace, not accumulate). However, **unsaved work is never silently discarded**: any **dirty** container in the session being left is **retained in memory** (not unloaded) and surfaced via the unsaved-work banner; **clean** containers from the prior session unload.

- **Unload.** *(Revised 2026-09-16.)* A container leaves panel + memory when it would no longer be shown in **any** viewport — after the user has chosen Save or Discard if it is dirty, silently if it is clean — **or** when the user explicitly deletes it (D7.6 row "✕"). Nothing stays loaded that renders nowhere; re-opening the scan reloads it. Accidental loss is prevented by the prompt rather than by indefinite retention.
...
#### D7.7 Ordering
- **No free-text filter / search and no sort control** — removed per review. Annotation counts per session are small.
- ~~**No "Active only" toggle**~~ — **superseded 2026-09-16.** The list is scoped to the focused viewport, so focus is the filter and a toggle would be redundant in a different way than intended: the original reasoning was that dimming + the cross-panel pill (D9) already conveyed active-viewport state, but neither was wired in any build. Both are wired now and kept alongside the scoping. See [`viewport-scoped-annotations-proposal.md`](viewport-scoped-annotations-proposal.md).
...
26. **Lifecycle: session switch + unsaved work (A13, E3).** *(Revised 2026-09-16 — was "unsaved retention".)* In session 1, edit a structure-set so it is **dirty**. Without saving, select a scan from session 2 in the browser: a **Save · Discard · Cancel** prompt names the structure-set before anything changes. **Cancel** aborts the switch and drops nothing. **Save** persists it, then the switch proceeds and both sessions' containers unload. **Discard** proceeds without saving. Either way nothing is left listed while rendering nowhere, and returning to session 1 **reloads** its saved containers. (Restart mid-edit → recovery per E3.)
