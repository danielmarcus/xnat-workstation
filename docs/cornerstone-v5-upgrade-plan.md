...
- [x] **`SafePaintFillTool` silently did nothing on volume viewports** — it read the 4.x `Labelmap.volumeId`, absent on v5's stack layers. Now resolves the segment's layer like v5's own `PaintFillTool` (`resolveLabelmapForSegment` + `getOrCreateLabelmapVolume`, from the published `@cornerstonejs/tools/segmentation/*` subpath). The existing E2E could not see it (seeded a solid stroke, clicked inside, asserted the count did not *drop*; and it built the segmentation via the E2E-only volume-labelmap hook). Rewritten to drive the panel's create button, paint a closed ring and require the fill to add voxels — red on v5 before the fix.
- [x] **Undo after a contour union lost the first contour.** v5 unions overlapping same-segment contours and rewrites that stroke's history entry with `DefaultHistoryMemo.replaceCurrentMemo` (in place, no `push`). Per-container undo — the path whenever a container is active — is fed from `push`, so it kept the stale entry. The history tracking now also wraps `replaceCurrentMemo` and mirrors it into per-container history (`perContainerHistory.replaceTop`). New E2E `tools/undo-contour-union` (red first): draw A, union with B, toolbar Undo → A as drawn; Redo → union.
- [x] Undo patch otherwise valid: `HistoryMemo` ring fields (`ring/position/size/canUndo/canRedo`) unchanged; `replaceCurrentMemo` is the only addition.
- [x] Instance patches verified against 5.10.11 source: BrushTool `disableCursor`, RegionSegmentPlus `mouseTimer`, draw-loop `activateDraw`/`renderContourBeingDrawn` own properties (multi-viewport preview spec re-verified red with the patch off), Sculptor `referencedToolNames`, `AnnotationToPointData.register` (spline still not self-registered upstream — keep ours).
- [x] Cursor & binding contract specs unchanged and green (`cursor-matrix`, `tool-cursors`, `shift-nav-swap`, `tool-binding-leak`, `switch-bindings`).
- [ ] `toolService.ts` deletion — flagged as a separate task (dead path, nothing to port).
