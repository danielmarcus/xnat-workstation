 ...
 * SAME labelmap-on-MPR rendering already proven green by signal 3 (`tools/brush-mpr`).
...
const ev = <T,>(page: Page, fn: keyof E2EHooks, ...args: unknown[]): Promise<T> =>
  page.evaluate(
    ([f, a]) => ((window as unknown as Win).__XNAT_E2E__[f as keyof E2EHooks] as unknown as (...fnArgs: unknown[]) => unknown)(...(a as unknown[])),
    [fn, args] as const,
  ) as Promise<T>;
