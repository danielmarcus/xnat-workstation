...
| `ct-axial-anatomy/` | Synthetic anatomy CT (soft-tissue ellipsoid + bone insert + Gaussian noise), 30 slices | intensity-aware tools (RegionSegment, ThresholdBrush, Paint Fill on real anatomy) | requirements §G #21 (positive case) |
...
- **Source**: synthetic. Generated by [`scripts/generate-synthetic-fixture-ct-anatomy.mjs`](../../../scripts/generate-synthetic-fixture-ct-anatomy.mjs).
- **Construction**: one axial CT series, 30 slices, 128×128 px, 16-bit signed. Soft-tissue ellipsoid (≈40 HU ± 10 HU Gaussian noise) centered in-volume; bone insert (≈800 HU ± 50 HU) offset inside the soft-tissue region; air background (−1000 HU ± 5 HU). Two-voxel-wide gradient blends between tissue classes so boundary clicks don't sample wildly bimodal voxels. Deterministic seeded RNG (mulberry32) keeps output bit-identical across reruns.
- **Why a separate fixture from `ct-axial-300/`**: the sphere phantom there is binary (0 inside, −1000 outside) — GrowCut-style region-grow degenerates (zero std dev inside ⇒ empty grow; huge std dev across the cliff ⇒ unbounded grow). `ct-axial-anatomy/` provides the realistic noise + smooth gradients those tools assume.
- **Verified anatomy** (re-derive by `node -e` on `slice015.dcm` per the spec): center pixel (64,64) ≈ 40 HU, 5×5 patch std dev ≈ 7–10 HU; bone insert at ≈ (73,60) ≈ 800 HU; air corner ≈ −1000 HU.
- **Verified metadata shape**: single `SeriesInstanceUID = 1.2.826.0.1.3680043.10.1338.997.2`; shared `FrameOfReferenceUID = …997.3`; `Modality = CT`; 30 slices; `RescaleType = HU`.
- **PHI**: none. `PatientName = Synthetic^Anatomy`, `PatientID = XNAT-WS-SYNTH-003`, `Manufacturer = XNAT-WS-SYNTH`.
- **Storage**: Git LFS.
