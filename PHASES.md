...
- ⏳ **Local DICOM fixtures** (e2e/fixtures/dicom/) — *Item 4*: harness landed in Phase 1 (loader + 10 unit tests for discovery). Helper-level pipeline acceptance landed (`e2e/specs/11-fixture-cross-series.e2e.ts`): discovers a fixture, parses files with dcmjs, validates the metadata-shape claim each fixture is named for; skips cleanly when no fixture is on disk. **Renamed** the design's `breath-hold-pair` slot to `sameforuid-different-acquisition` since the A2c heuristic keys on metadata (shared FoR + differing `AcquisitionNumber`), not anatomy displacement; same fixture covers the design's `4dct-phases` scenario. **Outstanding**: (a) two real fixtures still need to be acquired (TCIA candidates: small T1+T2 MR exam, 4D-CT phase pair) and committed under a chosen storage strategy (LFS / repo / external root); (b) renderer hook to mount local-file paths into a viewport so the spec can drive canvas-level assertions for signals 9 (A2b dashed stroke) and 10 (A2c off-by-default); (c) the other six fixture sets remain pending and will be picked up as Phase 2 work demands them.
