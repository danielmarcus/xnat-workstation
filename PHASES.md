...
**Status (2026-05-03, in progress)**: data layer + UI shell + visibility cycling + selection model + row hover + member CRUD + action menu landed (3.1 → 3.6). Test suite at 957 passing. Legacy panels still mount under flag-off; ContainerListPanel mounts alongside under flag-on for verification. Outstanding sub-phases: 3.7 (A2c opt-in + filter/search/sort + empty/loading states), 3.8 (approval UI + ROI type + provenance + E2E for signals 18/19/20/22), and the bidirectional canvas ↔ row hover sync (Phase 3.5b reciprocal — staged as its own follow-on because it requires Cornerstone tool internals).
