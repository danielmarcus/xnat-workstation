...
> - [ ] **C6** 4D / multi-volume functional images render wrong off-axis (sagittal/coronal). OPEN — the unified path packs all volumes of a 4D series into one ImageVolume → corrupt geometry. Needs proper 4D handling (time-point select + scrubber, or a true 4D volume). Architecture-level.
