...
- **TR5 (mocked-XNAT approach, chosen 2026):**
  ...
  - **🔒 Genuinely remaining (CNDA live-verify only):** SR-write `xsiType` (`xnat:srScanData`) confirmation on the live server — the SEG conflict/round-trip is verified; confirm an SR (Measurement) save round-trips + its scan type is accepted. (Plus #74's broader upload-path end-to-end sweep.)
