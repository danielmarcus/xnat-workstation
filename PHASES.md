...
- **Slices:** R3.1 container projection (pure data layer) → R3.2 active-member + selection model (state) + **unblocks Phase-2 deferred halves** (gesture block via `canDrawOnViewport(active)`, toolbar undo via `undoContainer(active)`) → R3.3 panel shell + header + Annotate toggle → R3.4 container rows → R3.5 member rows → R3.6 context toolbox → R3.7 dialogs + empty/loading/error → R3.8 mount + delete legacy `SegmentationPanel`/`AnnotationListPanel` + pixel-diff E2E + acceptance signals.
  ...
  - **Live panel acceptance E2Es ✅** — spec 31 (`0802d3d`, open→create→two-step edit→toolbox), spec 32 (`27a4696`, signal 33 selection: single-click replace / shift-click set / selection⊥active), spec 33 (`39633ad`, signal 35 D1/D3: toolbox offers only the active kind's tools, planned tools disabled).
  - **🔜 Remaining Phase-3 signals (genuinely gated):** 17/19/20/22 already green (Phase 1). 35 keyboard-scoping half (D5 global vs per-panel-view shortcuts) needs the focus/hotkey model. 25/26 (auto-load lifecycle) need an auto-load E2E. 27 (conflict) needs the XNAT transport. 32 (SR container create) needs the SR-create feature (currently a TODO). Each needs a feature/transport that isn't built — not just an E2E.
