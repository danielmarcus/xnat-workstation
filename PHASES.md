...
>
> **❌ Spec-required but missing/broken:** ~~info overlay~~ ✅B1; ~~event→store sync~~ ✅B1; ~~generic grid layouts~~ ✅B2; ~~multi-scan per panel~~ ✅B2; ~~world-point crosshair~~ ✅B3 (gap+sync confirmed); ~~scrollbar~~ ✅B4; ~~orientation control (axial→sag/cor)~~ ✅C2; ~~native-plane open~~ ✅C2; ~~orientation markers~~ ✅C3. **STILL OPEN:** rulers (scale bars — accuracy is DPR-sensitive/reconstruction-fragile, can't verify offline); brush-size control; error/loading overlay; MPR 4th-panel-as-3D.
>
...
> - [ ] **C4** Rulers (scale bars). **DEFERRED — verification gap:** the on-screen mm/pixel scale is either a fragile fit-scale reconstruction or DPR-sensitive `canvasToWorld` math; accuracy can't be asserted offline, and a wrong ruler misleads. Needs a real-data accuracy check or a true-scale source from Cornerstone before shipping.
> - [ ] **C5** Brush-size control · error/loading overlay · MPR 4th-panel-as-3D — open gaps needing design/scope decisions.
