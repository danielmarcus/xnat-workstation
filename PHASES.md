...
- **TR5 (mocked-XNAT approach, chosen 2026):**
  ...
  - **Conflict detection ✅ `5c589fb` (client-side, #64-A):** **FINDING — real XNAT has no native optimistic concurrency.** `overwriteDicomSegInScan` (+ RTSTRUCT/SR) take no base token and DELETE+PUT unconditionally (last-write-wins); the base token never reached the server. So H5/H7 only ever fired in the mock. Now made real on the CLIENT: `getVersion`/H6 is implemented end-to-end (`xnatClient.getScanVersionToken` GETs the stored file → token in `deriveVersionToken`'s scheme; `XNAT_GET_SCAN_VERSION` IPC; renderer `getVersion`), and `xnatTransport.save()` does a pre-overwrite poll-and-compare → conflict on mismatch (no overwrite), degrades to last-write-wins on null/no-base. Verified against the mock + unit tests.
  - **🔒 Genuinely remaining (CNDA live-verify only):** (1) **empirical gate** — does CNDA return a token comparable across our upload and a later GET? `deriveVersionToken` notes file responses rarely carry ETag/Last-Modified, so the common path is sha1-of-bytes, which matches only if XNAT stores bytes verbatim (no re-encode). If it re-encodes → false conflicts; fix = base the token on a post-save `getVersion`. (2) live conflict (signal 27) + external-edit detection round-trip on CNDA. (3) SR-write `xsiType` (`xnat:srScanData`) confirmation on the live server.
