...
>
> **Rebuild status (2026-07-30):** Phases 0–5 ✅ · Transport ✅ code-complete + CNDA-live-verified · Phase-3 list panel ✅ built/mounted (behind `REBUILT_ANNOTATIONS_PANEL`, on by default) · toolbar rebuilt to the frozen §10 mockup + polish (star favorites, ghost Annotate-when-closed, distinct rotate icon, modality-scoped W/L presets, organized responsive collapse) · Measurement/SR round-trip + UI bug-fix pass done. **Remaining before merge:** the **legacy-panel cutover** (Phase 6). A code-grounded parity audit (2026-07-30) found the rebuilt panel is close: save trigger + per-row status are wired, brush-size + CSV stats are done. The real cutover work is small — feed the autosave/backup **status text**, verify local-disk-save parity, then delete `SegmentationPanel` + `AnnotationListPanel` and drop the `REBUILT_ANNOTATIONS_PANEL` + (vestigial) `multiviewportEnabled` flags. Approval persistence + inline per-segment stats are net-new spec features that can ship *after* cutover. Plus the **CNDA-gated live-verification residuals** (#74 upload E2E, signals 25/26 browser-driven auto-load, #64-H6).
...
- **Slices:** R3.1 container projection (pure data layer) → R3.2 active-member + selection model (state) + **unblocks Phase-2 deferred halves** (gesture block via `canDrawOnViewport(active)`, toolbar undo via `undoContainer(active)`) → R3.3 panel shell + header + Annotate toggle → R3.4 container rows → R3.5 member rows → R3.6 context toolbox → R3.7 dialogs + empty/loading/error → R3.8 mount + delete legacy `SegmentationPanel`/`AnnotationListPanel` + pixel-diff E2E + acceptance signals.
  ...
  - **🔜 Legacy cutover — DEFERRED until parity (not punted):** delete `SegmentationPanel`/`AnnotationListPanel` + remove `REBUILT_ANNOTATIONS_PANEL`. Held because the new panel still has graceful-TODO gaps the legacy panel covers (save-to-XNAT, approval persistence, brush-size control, segment statistics) — all transport/Phase-gated; the flag is the fallback, so deleting now would regress. Cut over once those reach parity.
...
### Rebuild Phase 6 — Legacy-panel cutover + flag removal (🔜 Next milestone)

> **Scope corrected 2026-07-30 by a code-grounded parity audit.** The old "parity gap" list (save-UI / approval / brush-size / segment-stats) was inherited from a stale Phase-3 note and is mostly **already closed or never was a legacy gap**. The rebuilt panel is much closer to cutover than the list implied. Findings (with the audit's file:line refs):

- **Save-to-XNAT — mostly DONE.** The manual-save *trigger* and per-row status **are wired**: per-container Save (`ContainerRow.tsx` → `onSaveContainer` → `segmentationService.flushContainerSave` → `saveQueue.flush`), save-all via `ReviewUnsavedDialog`, per-row reflection from `useTransportStore`, H7 conflict dialog. Transport injected on mount (`useXnatAutosaveOptIn` → `composeXnatTransport` → `setSaveTransport`); manual Save is not opt-in-gated. New-vs-overwrite is auto-routed by the transport (H8 scanId mapping), so the legacy `ExistingSaveDialog` isn't needed. **True remaining gap = the autosave/backup *status text*:** `ContextToolbox` renders a `backupStatus` prop but `useAnnotationsPanel` never feeds it. Small wiring (derive from `useTransportStore` / `segmentationStore.autoSaveStatus`). Also confirm local-disk save parity (legacy per-row "Save file" vs the rebuilt "Export to DICOM…" kebab).
- **Approval persistence — NOT a legacy-parity gap.** The legacy panels have no approval affordance at all; approval is a *net-new spec feature* (D7.11). The rebuilt UI exists but `onApproveToggle` is a warn-only no-op, `approvalOf` is never passed to `projectContainers`, the approve/revoke `ConfirmDialog` is unmounted, and there is **no DICOM ApprovalStatus persistence layer**. This is a transport-workstream build-out → **can ship after cutover; it does not block deleting the legacy panels.**
- **Brush-size — already DONE.** The rebuilt `ContextToolbox` has the brush-size slider (→ `unifiedToolService.setBrushSize`). (There is *no* interim toolbar `BrushControl` — that was removed.) Residual **bug (not a cutover blocker):** three unsynced `brushSize` stores — the slider uses local hook state, the `[`/`]` hotkeys + `toolService` reactivation use `segmentationStore.brushSize`, and `viewerStore.brushSize` is dead. Consolidate onto one store.
- **Segment statistics — rebuilt EXCEEDS legacy.** Legacy exposes no stats/CSV. Rebuilt has container-level CSV export (kebab → `getSegmentStatistics` + `buildContainerCsv`: voxelCount/volumeMm3/mean/min/max/stdDev HU). Optional nice-to-have: inline per-segment volume/HU in `MemberRow` (compute path already exists). Not a parity blocker.

**Actual cutover work:**
1. Feed `ContextToolbox.backupStatus` (the one real save-UI parity gap) + confirm local-disk save parity.
2. Handle **both** legacy panels: `SegmentationPanel` (Segment toggle, behind `REBUILT_ANNOTATIONS_PANEL`) **and** `AnnotationListPanel`, which renders independently via a **separate** `useAnnotationStore.showPanel` toggle — verify the rebuilt panel covers its function before removing.
3. Delete `SegmentationPanel` + `AnnotationListPanel` (+ their child dialogs `segmentation/ExistingSaveDialog`,`DeleteConfirmDialog`, `segmentationPanelUtils`, and the two panel tests); remove `REBUILT_ANNOTATIONS_PANEL` (single consumer: `ViewerPage.tsx:19,120`).
4. Remove the **vestigial** `multiviewportEnabled` flag — no shipping code branches on it (`selectMultiviewportEnabled`/`isMultiviewportEnabled` have zero runtime callers); only comments + E2E hooks reference it. Note the `?? false` fallback in `selectMultiviewportEnabled` disagrees with the persisted `true` default (moot once removed).
5. (Recommended, separate) fix the three-store `brushSize` desync.
- **Deferrable past cutover (net-new spec, not legacy parity):** approval persistence (needs DICOM ApprovalStatus write) · inline per-segment stats.
- **Acceptance:** backup-status fed, both legacy panels + child dialogs deleted, `REBUILT_ANNOTATIONS_PANEL` + `multiviewportEnabled` gone, clean codebase, no flag remnants.
