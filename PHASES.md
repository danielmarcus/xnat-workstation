...
- **TR5 🔒 backend/live-XNAT** — the REAL transport: `saveTransport` calls `XNAT_UPLOAD_DICOM_SEG`/`_OVERWRITE_*` (handlers exist); first-save maps session id → XNAT scanId + token (H8); external-change polling (H6); round-trip-proven (F5). Needs a live/mocked XNAT; the production edit path must also drive `notifyContainerDirty` (Phase-3 gesture interceptor) + user opt-in before autosave-to-XNAT is live. `backupService` local-fs autosave stays the live persistence until then.
