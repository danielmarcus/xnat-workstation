...
> - [x] **C6** 4D / multi-volume functional images render wrong off-axis. ✅ `275bcba` (reworked from `1599428`). First attempt routed to the dynamic loader via getDynamicVolumeInfo — FAILED on the user's EPI perfusion series (CS only detects vendor-tagged cardiac/diffusion 4D; the dynamic loader re-runs the same failing split). Reworked to a tag-agnostic GEOMETRY fix: `selectPrimaryTimepointImageIds` (repeated ImagePositionPatient ⇒ 4D ⇒ keep the first image per position) → `volumeService.acquire` builds a clean 3D volume from ONE time point → off-axis reformat works, no more "No imageId found". Verified: volumeService.test (both orderings / 3D passthrough / missing-geometry / acquire-reduces) + 3D E2Es 25/28/29 unchanged. ⚠️ RENDERING fix only — shows the FIRST time point in all planes; confirm on real perfusion (diagnostic: console "[volumeService] 4D / multi-volume series → first time point: N of M images"). **Follow-up: time-point navigation (scrubber across volumes).**
