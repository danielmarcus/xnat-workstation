...
> **Current focus:** post-cutover follow-ups on branch `annotation-followups` (merged from `annotation-cleanup`). The historical product phases 0–12 below are complete/in-progress as marked. The rebuild's own phase plan lives in the **Active Work** section (after Phase 12); its phase numbers are scoped to the rebuild. Specs are in `docs/`.
>
> **Rebuild status (2026-09-01): Phases 0–6 ✅ COMPLETE + the deferred spec features closed.** `annotation-cleanup` merged to `main` (fast-forward, 216 commits) after the **legacy-panel cutover (Phase 6)**: the rebuilt Annotations side panel is the ONLY annotation surface — `SegmentationPanel` + `AnnotationListPanel` are deleted and both the `REBUILT_ANNOTATIONS_PANEL` and `multiviewportEnabled` flags are gone. Transport ✅ code-complete + CNDA-live-verified.
>
> **Then closed (the "deferred / gated" list, in order):**
> 1. **Inline per-segment volume** in the member rows (`f716bbd`) — the statistics path existed and fed the CSV export, but the row's metric slot was always blank.
> 2. **Approval workflow + DICOM persistence (D7.11)** (`9ca9cca`) — `ApprovalStatus (300E,0002)` + `Reviewer*` written on every export path and read back on load; approve/revoke confirmations; a hard edit lock at the draw gate.
> 3. **SR reload from an XNAT scan click** (`ca6fc78`) — the Measurement round-trip's missing half; upload was already CNDA-closed.
> 4. **Signals 25/26 auto-load** (`4c869f8`) — the browser-driven trigger now has offline coverage through the real `loadFromXnatScan`.
> 5. **D8 perf baseline** (`f1e3b8a`) — a real 300-slice/512² harness (`npm run test:perf`) with measured numbers in [`docs/perf-baseline.md`](docs/perf-baseline.md).
> 6. **MPR 4th-panel-as-3D (C5c)** (`89a3b65`) — a real VOLUME_3D rendering with its own rotate tool group and no slice chrome.
>
> **Genuinely remaining:**
> - **Incremental loading** — blocked on server-side WADO-RS. Nothing to do here.
> - **CNDA-only confirmations** (the code is written and offline-proven; only a live server can close them): does XNAT preserve `ApprovalStatus` through upload/download (private-tag fallback documented in the design if not) · the SR reload's real IPC download + a DOM click on a browser row · signals 25/26 against a live session.
> - **D8 on clinical hardware** — the layout-change metric straddles its 250 ms target on the dev Mac (181/344/534 ms across runs) and is explicitly not settled; edit propagation clears it at 52–57 fps.
> - **3D preset refinement** — CT-Bone is HU-calibrated, so synthetic phantoms render faintly; per-series or user-selectable presets would be the next step (not part of C5c).
...
- ✅ **Walking skeleton GREEN** through the real stack (Electron + WebGL2 Cornerstone + real fixture), offline, flag on: `e2e/specs/transport/walking-skeleton.e2e.ts`. Launch/WebGL2 smoke: `e2e/specs/smoke/launch-and-webgl2.e2e.ts`.
...
> - [x] **C5c** MPR 4th-panel-as-3D. ✅ `89a3b65` — the `mpr-2x2` preset's fourth slot is a Cornerstone VOLUME_3D rendering sharing the same (scanId, FoR) volume as the three slice planes, with a CT-Bone transfer function, its OWN tool group (`xnatToolGroup_volume3d`: trackball rotate + zoom/pan) and no slice chrome (no reformat dropdown / slice counter / thickness / W-L / edge markers / scale bar / scrollbar / time scrubber — the first capture showed all of those on a projected view). Verified by e2e/77 incl. a NON-BLANK-pixel assertion (a silently empty 3D canvas is the failure mode); e2e/14/15/18 updated off the old four-slice MPR. ⚠️ CT-Bone is HU-calibrated → synthetic phantoms render faintly; per-series/user-selectable presets are a later refinement.
...
- **Slices:** 1 FoR-eligibility service (pure) → 2 FoR-gated attach + non-native style (D9) → 3 gesture-start blocking → 4 per-container undo → 5 queue/debounced autosave → 6 copy/paste finish. **All six committed + live-wired; 753 unit tests green (117 files); tsc main 0 / renderer 59 baseline; build + lint clean; e2e spec 30 (signals 9/9b/10/11) green.**
  ...
  - **Two-panel cross-series pixel-diff harness — BUILT ✅ `d68e6be` (+ honest correction):** `loadTwoSeries` (e2e/helpers/local-fixture) + `e2e/specs/viewport/cross-series-two-panel` load two series of one study into a 1×2 grid and pixel-compare. Two tests green: harness sanity (two series render independently) + cross-series **render presence** (a SEG on the T1 panel appears on the same-FoR T2 panel). **Harness finding:** the eligibility-aware attach (`attachLabelmapWithEligibility`, Slice 2 — classify + per-viewport style) is **NOT wired into the live SEG-create/attach flow** (`createUnifiedLabelmapSegmentation`/`segmentationService.addToViewport` attach to ALL viewports unconditionally, the MPR behavior). So the same-FoR sibling shows the SEG at full opacity (not D9-dimmed), the displaced sibling isn't hidden (A2c), and a different-FoR panel still gets a non-rendering representation (A2d). Signals 9-dimming / 10 / 11 are recorded as `test.fixme` contracts in `viewport/cross-series-two-panel` — the harness drives them and will verify them once the eligibility attach is routed into the live flow. **This is the real remaining Phase-2 wiring** (bigger than "no harness").
  ...
  - **✅ Live-wiring CLOSED — all Phase-2 acceptance signals (1, 2, 8, 9, 10, 11, 12, 14, 15, 23) now landed:**
    ...
    - **Harness note (historical):** the two-panel pixel-diff harness (`d68e6be`, `loadTwoSeries` + `e2e/specs/viewport/cross-series-two-panel`) was the infrastructure the cross-series signals were blocked on; once `attachLabelmapWithEligibility` was routed into the live create flow (`6bb48a9`), signals 9/9b/10/11 went green there.
...
- **Slices:** R3.1 container projection (pure data layer) → R3.2 active-member + selection model (state) + **unblocks Phase-2 deferred halves** (gesture block via `canDrawOnViewport(active)`, toolbar undo via `undoContainer(active)`) → R3.3 panel shell + header + Annotate toggle → R3.4 container rows → R3.5 member rows → R3.6 context toolbox → R3.7 dialogs + empty/loading/error → R3.8 mount + delete legacy `SegmentationPanel`/`AnnotationListPanel` + pixel-diff E2E + acceptance signals.
  ...
  - **✅ Phase-3 signals closed:** 17/19/20/22 green (Phase 1). **25/26 ✅ `4c869f8`** — the auto-load TRIGGER is now offline-covered through the real `App.loadFromXnatScan` (e2e/76), with the scan list + derived file served through the `xnatScanApi` seam and the scan→imageIds cache primed with fixture ids; the retention + banner halves were already green (spec 39). A live-session run remains a CNDA confirmation, not a build item. 27 (conflict, H7) was closed by the transport workstream (CNDA-verified). The explicit "New Measurement (SR)" create-empty-set button + multi-SR-container routing stays deferred per the D7.1 measurement skeleton.
  - **✅ Approval workflow (D7.11) `9ca9cca`** — was "UI exists, behaviour doesn't": `onApproveToggle` warned, `approvalOf` was never projected, the dialog was unmounted, nothing enforced the lock. Now: pure `lib/annotations/approval` (DICOM `ApprovalStatus (300E,0002)` + `ReviewerName/Date/Time`, tolerant parse), `approvalStore` (record + session audit trail; seeding from a file records no audit entry), stamping in the ONE shared serializer so SEG/RTSTRUCT/SR all persist it (revoke DELETES the stale reviewer attributes), seeding on load for all three kinds (RTSTRUCT reuses its existing dicom-parser pass; SEG/SR use a lightweight `readApprovalFromArrayBuffer` that stops before PixelData), and enforcement at `canDrawOnViewport` (checked BEFORE spatial reasoning, so an approved container is locked on every viewport) plus handler-level backstops. Verified by 19 unit tests, a real dcmjs round-trip, and e2e/74. ⚠️ Whether a live XNAT preserves the attribute is CNDA-gated; the design's private-tag fallback stands.
  - **✅ Inline per-segment stats `f716bbd`** — `lib/annotations/segmentMetric` + `useSegmentMetrics` (expanded containers only, debounced on a new `segmentationStore.editEpoch`, stale-run guard) feed the member row's metric slot (mockup §3 "86 cm³"). e2e/73 paints with the real brush and asserts the row shows a volume.
...
- **Landed:** `2ff97bd` (Contour Fill — add the contour-edit prerequisite at tool activation) · `685e04a` (Contour Fill undo — bridge the missing history memo) · `15c1691` (fix Contour Fill on **oblique** acquisition planes — index-space rasterizer, `ct-oblique` fixture). Perf budget (D8): a real 300-slice/512² harness now MEASURES it (`npm run test:perf`, `f1e3b8a`) — edit propagation 52–57 fps across 4 panels (target ≥30) but layout-change 181/344/534 ms straddles the ~250 ms target on the dev Mac. Numbers + what they don't establish: [`docs/perf-baseline.md`](docs/perf-baseline.md).
...
- **TR5 (mocked-XNAT approach, chosen 2026):**
  ...
  - **✅ SR XNAT-scan-click reload WIRED `ca6fc78`** — the `isSrScan` branch mirrors RTSTRUCT: download → `lib/dicom/srReferences.getSrReferenceInfo` (a TID-1500 SR carries its linkage in CurrentRequestedProcedureEvidenceSequence (0040,A375), not a top-level ReferencedSeriesSequence; PertinentOtherEvidence and the content tree are fallbacks) → attach to an open panel showing that series, else find+load the source scan, else the active panel with a warning → `loadMeasurementsFromArrayBuffer`, recording the origin so a later save overwrites the same SR scan (H8). Repeat clicks no longer duplicate: the SR container carries the `xnatScanId` it came from (the origin map can't be the guard — an unresolvable source scan means no origin entry, which the test caught). Offline-proven end-to-end by e2e/75 against a REAL SR the app exported, driving the production `loadFromXnatScan` with the `xnatScanApi` seam faked. 🔒 The live IPC download + a DOM click on a browser row remain CNDA confirmations.
