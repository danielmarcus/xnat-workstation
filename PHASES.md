...
- **TR5 (mocked-XNAT approach, chosen 2026):**
  ...
  - **🔒 Genuinely remaining (small, specific):** (1) `getVersion`/H6 external-version polling returns `null` — needs a light HEAD/catalog IPC to detect a server-side change before save; (2) RTSTRUCT/SR FIRST-save upload routing — `XnatUploadApi.uploadSeg` always uploads via the SEG channel (overwrite already routes RTSTRUCT via `overwriteKindOf`); SR write not handled; (3) live conflict (signal 27) verification on CNDA.
