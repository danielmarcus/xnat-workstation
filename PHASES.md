...
- **P1.7 — Turn signals 1/3/6/7 green (real-Cornerstone E2E, visual).** Decision 2026-06-08 (user): the four signals all require annotation/segmentation **editing** that pure viewport-unification excludes, so the minimal editing/undo/dirty-save needed to satisfy them is **pulled forward into Phase 1** (rather than re-scoping). Reuse the existing `segmentationService` machinery (brush/contour/SEG/undo via global `DefaultHistoryMemo`/dirty flag/`exportToDicomSeg`) — it uses **viewport-focused** Cornerstone APIs + a **global** history, so it wires onto the unified tool group + viewports with minimal new code. Ordered by capability dependency (each red→green, commit per slice):
  ...
  - **P1.7b — signal 3: brush SEG → MPR resampled (native labelmap).** Create a labelmap SEG on the shared volume + add the representation to the unified MPR viewports; brush-paint via a real gesture on one panel; assert the painted voxels appear resampled on the other MPR panels (one 3D labelmap rendered on every plane — native, no conversion). (Stack+MPR coexistence framing per signal 3 to be decided during impl; the core claim is brush→resampled-on-MPR.)
  - **P1.7c — signal 7: undo a brush stroke from a closed panel.** Brush on a panel, switch layout so that panel is destroyed, undo (delegates to the global `DefaultHistoryMemo`), assert the stroke is undone. Wire the `undoService` skeleton to the real undo.
  - **P1.7d — signal 6: layout swap loses nothing + single dirty flag + save.** Create a structure, switch layouts 2×2→1×1→MPR→2×2 rapidly, assert no loss/duplicate/stale + single `hasUnsavedChanges` flag; `exportToDicomSeg` once produces a SEG that parses + validates offline (Rows/Columns/segments consistent).
  - **P1.7e — signal 1: freehand contour live on sagittal/coronal (PolySeg).** With editing infra proven, wire live PolySeg contour→labelmap so a freehand contour drawn on axial resamples onto the sagittal + coronal panels. (Drawing itself already validated; this slice is the cross-plane rendering.)
