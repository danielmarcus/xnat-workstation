...
### MV-Phase 6: Legacy removal + flag flip (In progress — opened 2026-05-03; 6.3, 6.4, 6.5, 2B.1, 2B.3 landed 2026-05-03; 2B.2/2B.4/6.6/6.7 deferred)
...
- ⏳ **6.2** Remove legacy panel mounts. Realized via Stage 2B (option 2B from the gap analysis) — see Stage 2B sub-tasks below. **Status: partial — 2B.1 + 2B.3 landed; 2B.2 + 2B.4 deferred** (Upload-to-XNAT extraction is ~300 LOC of new service module work entangled with toast/dialog state; doing it badly would regress critical user workflow). Once 2B.2 lands, 2B.4 can delete `SegmentationPanel.tsx` (2,054 LOC) and `AnnotationListPanel.tsx` (95 LOC) and the flag's panel-gating role with it.
...
- ⏳ **6.6** Delete the `multiViewport.enabled` flag itself. **Deferred**: the flag's main remaining role is gating the side-panel mounts (`SegmentationPanel` vs `ContainerListPanel`). Until 2B.2 + 2B.4 land and `SegmentationPanel` deletes, removing the flag would either show both panels simultaneously (bad UX) or strip XNAT-upload functionality entirely (regression). Pre-2B.4 sequencing: do 2B.2 → 2B.4 → 6.6 → 6.7 in one session.
- ⏳ **6.7** Final regression sweep: full Playwright suite + service-integration suite green on the legacy-removed branch. Documented walkthrough of all 22 G-signals. **Deferred** until 6.6.
...
- ⏳ **2B.2** Move "Upload to XNAT" into ContainerListPanel's per-container ⋯ menu. **Deferred.** `handleUploadXnat` in [SegmentationPanel.tsx:1016-1239](src/renderer/components/viewer/SegmentationPanel.tsx#L1016) is ~210 LOC tightly coupled to React-local state: toast notifications, save-menu dropdown, the existing-save dialog (overwrite-vs-create-new with versioned-label suggestion), `segmentationManager.beginManualSave/endManualSave` bracketing, post-upload session-scan refresh + UID-association rebuild, backup-entry cleanup, legacy XNAT temp-file cleanup. Cleanly extracting all of this into a `containerActions.uploadContainerToXnat(containerId, deps)` requires a new service module of comparable size with a richer DI surface (toast/dialog callbacks, manager-singleton hooks, backup-service hook, electron-API hook). Doing it badly would regress the existing-save dialog (every save would silently create a new scan, polluting XNAT projects). Sequenced as a single-session focused effort post-Phase-6 closure work.
...
- ⏳ **2B.4** Delete `SegmentationPanel.tsx` + `AnnotationListPanel.tsx` + `useSegmentationStore.showPanel` + `useSegmentationStore.activeSegTool` (and the `setActiveSegTool` action) + the `useAnnotationStore.showPanel` + `togglePanel`. Blocked on **2B.2**. After 2B.4, ContainerListPanel becomes the sole side-panel surface; the toolbar handles tool activation + brush size + Add buttons; the per-container ⋯ menu handles Save / Revert / Export / Upload to XNAT.
...
#### Phase 6 status (2026-05-03)

**Landed**: 6.3 (StackViewport rename), 6.4 (legacy MPR path removal), 6.5 (Crosshairs flip retired with rationale), 2B.1 (Add buttons in toolbar), 2B.3 (brush slider in dropdown).

**Deferred to a follow-up session**: 2B.2 (Upload-to-XNAT extraction → ~300 LOC service module), 2B.4 (delete SegmentationPanel — blocked on 2B.2), 6.6 (delete flag — blocked on 2B.4 to avoid showing both panels), 6.7 (final regression sweep — blocked on 6.6).

**Test count**: 1250 vitest passing (was 1275 at end of Phase 5; 27 deleted with the MPR code, 3 added for AddAnnotationButtons, baseline minor adjustments).

**Honest forward-looking note**: Phase 6 closure is ~80% mechanical cleanup, ~20% an upload-flow service refactor that needs its own focused PR. The work that remained at session end is well-scoped — 2B.2 needs a fresh commit window with the extraction plan in mind; 2B.4 / 6.6 / 6.7 cascade in minutes once 2B.2 lands.
