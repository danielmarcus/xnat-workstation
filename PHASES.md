...
**Status (2026-05-03)**: data layer + UI shell + visibility cycling + selection model + bidirectional hover sync (row ↔ canvas-contour) + member CRUD + action menu + filter/search + A2c opt-in + sort + approval UI + ROI type / provenance + inline ROI type editor + signal 18/19 service-integration + service-layer edit-lock when approved landed (3.1 → 3.5c-canvas-contour + 3.6 + 3.7 + 3.8a–e). Test suite at 1072 passing. Legacy panels still mount under flag-off; ContainerListPanel mounts alongside under flag-on for verification. Outstanding sub-phases: 3.5c-canvas-labelmap (cursor-over-labelmap-voxel direction — needs Cornerstone voxel sampling), 3.8f (Playwright E2E once fixtures land), and signal 22 (provenance round-trip via interpolation — Phase 4 territory).
