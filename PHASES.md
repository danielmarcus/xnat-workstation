...
| 06-save-upload.e2e.ts | 1 | 0 | 1 | 0 | "upload segmentation to XNAT" — Save button stays `disabled` because the freshly-painted SEG is not flagged dirty. ⚠️ defensive fix only (2026-05-03): the per-segmentation suppression deadlines that suspect-list called out were shrunk from 400ms (addToViewport) and 600ms (restorePresentationState) to 50ms each — long enough for Cornerstone's microtask + setTimeout(0) phantom events but short enough that user input ~100ms later can't race them. Repro requires live XNAT and is not validated here; if Bug 2 (no labelmap pixels written, upstream of Bug 1) is the real cause the test still fails. |
...
**Follow-up (2026-05-03)**: 04 and 05 are now ✅ resolved (see updated Notes column above). 06 has a defensive structural fix for the documented Bug 1 suspicion (suppression-window shrink) but cannot be validated without live XNAT; if Bug 2 (no labelmap pixels written) is the actual cause, the test still fails.
