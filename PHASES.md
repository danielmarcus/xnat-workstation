...
- ✅ **Acceptance — performance budget** (4-panel CT load ≤ baseline + 30%): instrumented E2E in [`e2e/specs/15-performance-budget.e2e.ts`](e2e/specs/15-performance-budget.e2e.ts). Loads the local CT fixture into 4 panels (warm-up + cold-mount), times from `loadLocalDicomFiles` start to all 4 canvases visible, asserts ≤ 6000 ms (env-overridable via `XNAT_E2E_PERF_BUDGET_MS_4PANEL`). Default budget is the +30% bound around the empirical baseline observed on a mid-tier 2024 macOS laptop. Always logs the measured value so CI baselines can be observed and the budget tightened intentionally.
- ✅ **Local DICOM fixtures** (e2e/fixtures/dicom/) — *Item 4*: harness landed in Phase 1 (loader + 10 unit tests). Pipeline acceptance landed in [`e2e/specs/11-fixture-cross-series.e2e.ts`](e2e/specs/11-fixture-cross-series.e2e.ts): discovers each fixture, parses files with dcmjs, validates the metadata-shape claim each fixture is named for; skips cleanly when a fixture isn't on disk. Spec is 8 / 8. **Renamed** the design's `breath-hold-pair` slot to `sameforuid-different-acquisition` since the A2c heuristic keys on metadata (shared FoR + differing `AcquisitionNumber`), not anatomy displacement; same fixture covers the design's `4dct-phases` scenario. **Storage**: Git LFS via `.gitattributes` at the repo root. **Renderer hook** `__XNAT_E2E__.loadLocalDicomFiles(panelId, paths[])` lands fixture DICOMs into a panel through the production wadouri.fileManager + setPanelImageIds path with no XNAT round-trip; companion `setFakeConnected(bool)` opens the viewer gate for local-fixture specs. Spec drives the hook to mount the T1+T2 fixture on two panels and asserts both viewport canvases render. **All seven fixture slots populated** (~9 MB total via LFS):
    ...
    **Migration fixmes (6 tests skipped, all pre-Phase-2.7 territory):**
    ...
    - **Signal 10 (A2c off-by-default) classifier**: ✅ resolved 2026-05-03 by [`acquisitionNumberProvider.ts`](src/renderer/lib/cornerstone/acquisitionNumberProvider.ts). New custom Cornerstone metaData provider for type `acquisitionNumberExtension` parses x00200012 from the wadouri-cached DICOM dataset; `cornerstoneVisibilityAdapter.identityFromImageId` consults it as a fallback when the wadouri `instance` aggregate's AcquisitionNumber is null/missing. Synthetic CT fixtures via `dicomfile:` now classify A2c correctly. E2E for signal 10 still ⏳ on the breath-hold fixture itself.
...
| 04-annotations.e2e.ts | 5 | 4 → 5 (post-fix) | 1 → 0 (post-fix) | 0 | "select annotation highlights it" — ✅ fixed 2026-05-03. Two-part repair: (a) `annotationService.selectAnnotation` now syncs Cornerstone's selection state via `csAnnotation.selection.setAnnotationSelected` (so a subsequent ANNOTATION_SELECTION_CHANGE doesn't clobber the store with an empty selection); the sync is idempotent — guards against the cs-side recursion; (b) `AnnotationListPanel` click handler is always-select (not toggle-on-re-click) so a click after the canvas auto-selected the new annotation confirms the selection rather than dropping it. |
| 05-segmentations.e2e.ts | 6 | 5 → 6 (post-fix) | 1 → 0 (post-fix) | 0 | "click contour, ctrl-c/ctrl-v, undo/redo and interpolation" — ✅ fixed 2026-05-03. Manually ported from the unmerged `fix/contour-paste-interpolation-uid` branch (cherry-pick conflicted across the multiviewport-annotation rewrite). New [`contourClipboard.helpers.ts`](src/renderer/lib/cornerstone/segmentationService/contourClipboard.helpers.ts) `resolveContourChainInterpolationUIDWithDeps` finds-or-mints an interpolationUID for the chain and backfills siblings. Spec adds the `isViewportImagePlaneMetadataReady` poll before paste so the metadata-preloader race doesn't mask the fix. 8 helper unit tests pin the contract. |
| 06-save-upload.e2e.ts | 1 | 0 | 1 | 0 | "upload segmentation to XNAT" — Save button stays `disabled` because the freshly-painted SEG is not flagged dirty. **Bugs 1 + 2 audited 2026-05-03** — [`19-brush-pixel-write-audit.e2e.ts`](e2e/specs/19-brush-pixel-write-audit.e2e.ts) drives the same Add seg → Brush → paintStroke flow on the local CT fixture in both flag-off and flag-on, then asserts (a) `hasUnsavedChanges === true`, (b) `dirtyIds` contains the seg id, (c) exported PixelData contains non-zero voxels. Both flag variants pass — the brush pipeline writes pixels AND flags dirty correctly. The Bug 1 / Bug 2 framing was stale; the suppression-window shrink + acquisitionNumberProvider + fixture-bridge XNAT-context-synthesis work this session collectively resolved both. The remaining 06-save-upload failure is XNAT-upload-specific (live XNAT only) — not a brush-or-dirty-tracking issue. |
...
**Follow-up (2026-05-03)**: 04 and 05 are now ✅ resolved. 06's brush pipeline is verified end-to-end by [`19-brush-pixel-write-audit.e2e.ts`](e2e/specs/19-brush-pixel-write-audit.e2e.ts) (both flag-off and flag-on): brush writes pixels, dirty flag sets, exported PixelData contains non-zero voxels. The remaining 06 failure is XNAT-upload-specific (live XNAT only). Bug 1 + Bug 2 are no longer suspect — they were misdiagnosed.
...
| G5 | Per-viewport hide of "GTV"; close panel → reopen resets to global default | MV-Phase 3 | ✅ pinned | New `containerService.setMemberVisibilityOnViewport(memberId, viewportId, visible)` (2026-05-03) routes one viewport's hide through Cornerstone's per-(viewport, segmentation, kind) `setSegmentVisibility`. The override lives in Cornerstone's representation state — closing the viewport destroys it, reopening starts from the global default per A5. UI affordance: alt+click on the visibility glyph in `ContainerListPanel` triggers per-viewport hide on the active viewport (regular click still cycles the D7.3 global mode). [`g5-per-viewport-hide.test.ts`](src/renderer/lib/cornerstone/__tests__/g5-per-viewport-hide.test.ts) wires 11 service-integration tests covering: hides target one viewport only, multi-kind iteration, no global-mode mutation, no dirty-flag mutation, no application-side persistent state, reattach simulation. |
...
| G7 | Undo after a brush stroke on a panel that has since been closed | **MV-Phase 1** | ✅ pinned | `e2e/specs/09-undo-after-close.e2e.ts` — both flag-off (legacy stack mode) AND flag-on (volume mode) variants pass as of 2026-05-03. The original "synthetic CT path doesn't render the row" / "volume-mode brush capability gap" framings were both wrong; the actual blocker was that `loadLocalDicomFiles` didn't synthesise a panel XNAT context, so `SegmentationPanel.visibleSegmentationIds` excluded freshly-created segs. Fixed in `e2eFixtureBridge.ts`. |
...
| G10 | Breath-hold pair (shared FoR, displaced anatomy); off-by-default; toggle "show structures from related series" | MV-Phase 2 | ✅ pinned | E2E ([`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts)) — A2c classifier + off-by-default `'hide'` action. AcquisitionNumber metadata gap fixed by `acquisitionNumberProvider.ts` (also handles the `dicomfile:` scheme prefix). |
| G11 | Cross-FoR CT+MR; structure-set hidden on MR viewport; list panel shows "different FoR" indicator | MV-Phase 2 | ✅ pinned | classifier + E2E ([`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts)) on the cross-FoR CT+MR fixture; UI indicator from D9 work. |
| G12 | Drawing on non-native series blocked at gesture-start with hint | MV-Phase 2 | ✅ pinned | drawing-routing 2.x + E2E ([`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts)) drives `shouldBlockDrawingOnViewport` against the T1+T2 fixture. |
| G13 | Interpolate every 5th slice; save without prompt; reload identical geometry | MV-Phase 4 | ✅ pinned | `multiviewport-phase4-integration.test.ts` + E2E [`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts) (write-through round-trip — three contours on three slices export-then-parse with identical geometry). |
| G14 | Autosave + queue-next-save under in-flight save; no edits lost | MV-Phase 2 | ✅ pinned | E2 queue-next-save coordinator in 2.x + E2E ([`18-save-flow-acceptance.e2e.ts`](e2e/specs/18-save-flow-acceptance.e2e.ts)) drives 5 rapid `flushNow` calls through a synthetic SaveAdapter; saves coalesce to ≤ 2 with `inFlight` peaking at 1. |
| G15 | Undo crosses save point; dirty flag re-asserts; new save flushes post-undo state | MV-Phase 2 | ✅ pinned | service-integration + E2E ([`18-save-flow-acceptance.e2e.ts`](e2e/specs/18-save-flow-acceptance.e2e.ts)) — synthetic SaveAdapter installed via `installTestSaveAdapter` exercises the full save → undo → re-dirty → save cycle through `transport.notifyDirty` / `flushNow`. |
| G16 | 3D paint-fill on axial appears resampled on sagittal MPR; one undo reverts entire fill | MV-Phase 5 | ✅ pinned (both halves) | Positive + single-undo: E2E [`23-g16-paint-fill-positive.e2e.ts`](e2e/specs/23-g16-paint-fill-positive.e2e.ts) drives the production Add seg → Brush boundary square → Paint Fill interior → Ctrl+Z flow; asserts the fill increment is non-trivial AND that one undo reverts to the brush-only baseline exactly. Unit-pinned by [`SafePaintFillTool.test.ts`](src/renderer/lib/cornerstone/tools/SafePaintFillTool.test.ts) "G16: a single fill records all voxel changes in one memo". Cross-MPR resampling follows from §C1 (G1 + G3 already pinned). |
...
| G18 | RTSTRUCT ROI types; inline edit; round-trip via `RTROIInterpretedType` | MV-Phase 3 | ✅ pinned | 3.8d service-integration + E2E ([`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts)) — both load (parse fixture) and round-trip (export → parse) paths. |
| G19 | Approve container; members edit-locked; persist via `ApprovalStatus`; revoke flow | MV-Phase 3 | ✅ pinned | 3.8a–e + 3.8e service-layer edit-lock + E2E ([`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts)) verifies `ApprovalStatus='APPROVED'` round-trips through DICOM. |
| G20 | Visibility modes filled / outlined / hidden; not persisted on reload | MV-Phase 3 | ✅ owning phase work landed | 2.9 + 3.x. |
| G21 | Region-segment / smart brush; lock blocks at gesture-start | MV-Phase 5 | ✅ pinned (both halves) | Lock-block half: E2E [`20-g21-region-segment-audit.e2e.ts`](e2e/specs/20-g21-region-segment-audit.e2e.ts) (panel-disable + pointerdown guard) + `SafePaintFillTool` G21-backstop test (the labelmap-editor symmetric defense). Positive half: E2E [`22-g21-region-segment-positive.e2e.ts`](e2e/specs/22-g21-region-segment-positive.e2e.ts) on the new `ct-axial-anatomy` fixture (soft-tissue ellipsoid + bone insert + Gaussian noise; GrowCut produces a bounded non-empty grow with realistic intensity statistics). The sphere-phantom `ct-axial-300` could not exercise this — binary 0/-1000 HU degenerates GrowCut. |
| G22 | Interpolation provenance round-trip (`interpolated` → manual on edit; survives save where DICOM permits) | MV-Phase 4 | ✅ pinned | `multiviewport-phase4-integration.test.ts` + E2E ([`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts)) — verifies the §D7.2 re-derivation contract: export does not encode provenance into a DICOM tag (by design); structural completeness of the round-trip enables on-load re-derivation. |
...
**Status (2026-05-03, Phase 1 closed)**: the earlier "end of Phase 1 work" framing was premature; the audit re-opened it on G1, G2, G8. All three are now ✅: G1 via [`13-g1-acceptance.e2e.ts`](e2e/specs/13-g1-acceptance.e2e.ts) (`mpr-2x2` cross-orientation contour visibility), G2 + G8 via [`12-g2-g8-acceptance.e2e.ts`](e2e/specs/12-g2-g8-acceptance.e2e.ts) (two stack panels on the same series, post-#75-loader-scoping). The performance budget is now pinned by [`15-performance-budget.e2e.ts`](e2e/specs/15-performance-budget.e2e.ts) — env-overridable budget defaulting to 6000ms, with the measured value logged for CI baseline observation. Volume rendering, shared-volume cache, oriented panels, MPR preset routing, full event surface on VolumeViewport, primary-group CrosshairsTool registration, plus the panel-scoped loader fix (issue #75) are all in place. Future phases should continue using per-scenario audit tables as exit gates rather than relying on aggregate playwright-suite counts.

### MV-Phase 2: Annotation behavior (Complete — signals 2/8/9/10/11/12/14/15 closed via E2E 2026-05-03)
...
**2026-05-03 audit**: Both bugs were misdiagnosed. Empirical E2E ([`19-brush-pixel-write-audit.e2e.ts`](e2e/specs/19-brush-pixel-write-audit.e2e.ts), both flag-off and flag-on) drives the production Add seg → Brush → paintStroke flow on the local CT fixture and asserts: `hasUnsavedChanges === true`, `dirtyIds` includes the seg, exported PixelData contains non-zero voxels. Both variants pass. The cumulative effect of (a) the suppression-window shrink (400ms → 50ms / 600ms → 50ms), (b) the acquisitionNumberProvider for `dicomfile:`, and (c) the e2eFixtureBridge XNAT-context synthesis resolved the visible symptom. The "isDirtyTrackingSuppressed returns true at brush time" + "labelmap not attached" framings were both downstream of the synthetic-CT setup gap.
...
- ✅ **A12 stress variant** ([`e2e/specs/14-a12-stress.e2e.ts`](e2e/specs/14-a12-stress.e2e.ts), 2026-05-03): three concurrent `loadLocalDicomFiles` calls overlapped with a 1×1 → 2×2 → 1×2 → 2×2 setLayout sequence + a structure-creation + post-create churn. End-state assertions: no duplicate segmentations (`getSegmentationSnapshot.length === 1`), no ghost panel records (every `activeByPanel` entry has `isCurrentLayoutPanel === true`), single dirty flag (`perSegmentationDirty.length === 1`).
...
| **9** | T1+T2 cross-series with dashed stroke | Phase 2.1/2.2/2.3/2.4 | service-integration ✅ + E2E ([`11-fixture-cross-series.e2e.ts`](e2e/specs/11-fixture-cross-series.e2e.ts)) | ✅ classifier + style action shape pinned. Visual dashed-stroke + hover-tooltip + handle-drag-blocked layers remain ⏳ on a pixel/DOM-overlay test (intentionally out of scope here). |
| **10** | Breath-hold A2c off-by-default | Phase 2.1/2.2/2.3 | service-integration ✅ + E2E ([`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts)) | ✅ — `acquisitionNumberProvider.ts` unblocked the classifier; E2E asserts `eligibility='cross-series-A2c'` + `action.kind='hide'` on the breath-hold fixture. |
| **11** | Cross-FoR not rendered, list visible | Phase 2.3 | service-integration ✅ + E2E ([`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts)) | ✅ — CT + MR cross-FoR fixture; classifier resolves `eligibility='cross-FoR'` + `action.kind='hide'`. |
| **12** | Drawing block on non-native viewport + hint | Phase 2.5a/2.5b | service-integration ✅ + E2E ([`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts)) | ✅ — drives `segmentationService.shouldBlockDrawingOnViewport` against the T1+T2 fixture; native viewport allows, cross-series viewport blocks with `reason='cross-series'` + non-empty hint. |
| **14** | Queue-next-save through rapid edits | Phase 2.8a/2.8b | service-integration ✅ + E2E ([`18-save-flow-acceptance.e2e.ts`](e2e/specs/18-save-flow-acceptance.e2e.ts)) | ✅ — synthetic SaveAdapter records call counts; 5 rapid `flushNow` invocations collapse to ≤ 2 saves, `inFlight` peaks at 1. |
| **15** | Undo past save re-dirties container | Phase 2.7a/2.7b + Phase 2.8 | service-integration ✅ + E2E ([`18-save-flow-acceptance.e2e.ts`](e2e/specs/18-save-flow-acceptance.e2e.ts)) | ✅ — drives the full save → edit → undo → re-dirty → save cycle through the production transport API; `bridge.dirty` toggles correctly across save points. |
| **G7** | Undo after panel close (cross-viewport identity) | Phase 1 (flag-off) + Phase 2.7 (flag-on path) | E2E ([09-undo-after-close.e2e.ts](e2e/specs/09-undo-after-close.e2e.ts) — both variants) + service-integration ✅ | ✅ — flag-off + flag-on both pass after the 2026-05-03 fixture-bridge fix. |
...
- ~~G7 flag-on (volume-mode brush + close + undo) is committed as `test.fixme`; the headline cross-viewport-identity signal does not run on the production rendering path.~~ Resolved 2026-05-03 — fixture-bridge XNAT-context fix unblocks both flag-on and flag-off variants. The "volume-mode brush capability gap" diagnosis was wrong.

Phase 2 deliverables (workstreams A + B) are functionally landed. Signals 2 / 8 close via the [issue #75](https://github.com/danielmarcus/xnat-workstation/issues/75) loader-scoping fix + [`12-g2-g8-acceptance.e2e.ts`](e2e/specs/12-g2-g8-acceptance.e2e.ts). Signals 9 / 10 / 11 / 12 close via [`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts) (the cross-series classifier is fully covered after the `acquisitionNumberProvider.ts` extension landed for the `dicomfile:` scheme). Signals 14 / 15 close via [`18-save-flow-acceptance.e2e.ts`](e2e/specs/18-save-flow-acceptance.e2e.ts) using a synthetic SaveAdapter installed through `__XNAT_E2E__.installTestSaveAdapter`. Phase 2 is closed at 2026-05-03.

### MV-Phase 3: List panel (Complete — signals 5/8/17/18/19 closed via E2E 2026-05-03)
...
| Signal | Description | Delivered by | Test layer | Status |
|---|---|---|---|---|
| **5** | Hide structure on panel A only; close panel and reopen → resets to global default per A5 | Phase 3.4 visibility plumbing + 3.x per-viewport hide (2026-05-03) | service-integration ([`g5-per-viewport-hide.test.ts`](src/renderer/lib/cornerstone/__tests__/g5-per-viewport-hide.test.ts)) | ✅ — `containerService.setMemberVisibilityOnViewport` routes the hide through Cornerstone's per-viewport API; alt+click on the eye icon in `ContainerListPanel` invokes it for the active viewport. Override lives in Cornerstone's representation state, so close-and-reopen reset is automatic (no application-side state to clear). |
...
| **18** | Load RTSTRUCT with typed ROIs; type badge per row; inline edit; round-trip via DICOM `RTROIInterpretedType` | Phase 3.8b/3.8c (badge + inline editor) + Phase 3.8d (service-integration) + RT export overlay (2026-05-03) | service-integration ✅ + E2E ([`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts)) | ✅ — load-side parses `rtstruct-typed.dcm` and confirms all canonical RTROIInterpretedType values present. Round-trip-side mutates a member's ROI type via `containerService.setRoiType`, exports through `rtStructService.exportToRtStruct`, parses the result, asserts the type round-trips. New `applyContainerMetadataToRtStructDataset` post-processing step overlays the bridge's typed-ROI value onto the adapter's generic 'ORGAN' default. |
| **19** | Approve container; members edit-locked; persist via DICOM `ApprovalStatus`; revoke flow | Phase 3.8a (UI) + 3.8d (round-trip) + 3.8e (service-layer edit-lock) + RT export overlay (2026-05-03) | service-integration ✅ + E2E ([`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts)) | ✅ — `applyContainerMetadataToRtStructDataset` writes `ApprovalStatus='APPROVED'` when the container is approved; round-trip parse confirms persistence. |
...
**Phase 3 closure gate**: signals 5, 8, 17, 18, 19 must all be ✅. Status as of 2026-05-03: **5 ✅** ([`g5-per-viewport-hide.test.ts`](src/renderer/lib/cornerstone/__tests__/g5-per-viewport-hide.test.ts)), **8 ✅** ([`12-g2-g8-acceptance.e2e.ts`](e2e/specs/12-g2-g8-acceptance.e2e.ts)), **17 ✅** ([`g17-active-member-append.test.ts`](src/renderer/lib/cornerstone/__tests__/g17-active-member-append.test.ts)), **18 ✅** + **19 ✅** ([`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts) drives the full save → parse round-trip; new `applyContainerMetadataToRtStructDataset` overlays bridge metadata onto the adapter output). Phase 3 closure gate satisfied.
...
The original closing paragraph framed Phase 3 list-panel work as "complete apart from 3.8f and signal 22." Audit-complete status (2026-05-03): G5 closes via `containerService.setMemberVisibilityOnViewport` + alt+click affordance + 11 service-integration tests. G17 closes via `g17-active-member-append.test.ts`. G8 closes via the same E2E spec that closes G2 (post-#75). Remaining ⏳ items belong to the RTSTRUCT save-load fixture deferral (signals 13 / 18 / 19 / 22 E2E).
...
- ✅ **4.8** Step-through interpolated slices review affordance (design §B5: "optional review affordance"). New `containerActions.stepThroughInterpolated(memberId)` action plus a small "▶" button on member rows alongside the existing AI marker, surfaced only when `interpolationState === 'has-interpolated'`. Each click advances the active viewport to the next slice that contains a contour for the member; wraps at the end. Pure helper `nextSliceIndex(current, sliceIndices)` factored out for unit testing. **v1 simplification**: navigates through ALL contour slices for the member (not just auto-generated ones) — the affordance only appears immediately after interpolation and any subsequent edit/save clears the marker, so in practice the contour set is dominated by interpolated slices. **Volume-mode landed 2026-05-03** — `readMemberContourSlices` now falls back to `BaseVolumeViewport.getImageIds(volumeId)` via `getAllVolumeIds()` when the 0-arg form returns empty, and `viewportService.scrollToIndex` handles volume viewports via `setSliceIndex` / delta scroll. The original "getImageIds returns empty on volume viewports" framing was stale — the volumeId-aware overload exists. 18 unit + component tests.
- ✅ **Playwright E2E for signals 13 / 22** ([`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts), 2026-05-03). The "deferred RTSTRUCT save-load fixture" framing was wrong: the round-trip happens entirely in the renderer via `exportToRtStruct → parseRtStruct`, no XNAT round-trip needed. G13 verifies three contours' geometry survives the round-trip; G22 verifies the §D7.2 re-derivation contract holds (no provenance vendor tag; structural completeness suffices).
...
- ~~Playwright E2E for signals 13 / 22 needs a synthetic RTSTRUCT round-trip fixture (the Phase 1 deferral).~~ Resolved 2026-05-03 — the round-trip happens entirely in the renderer; see [`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts).
- ~~Step-through review affordance (4.8) is stack-mode only; volume-mode review needs an alternate read-slices dep (volume viewports return empty `getImageIds()`).~~ Resolved 2026-05-03 — `readMemberContourSlices` now uses `BaseVolumeViewport.getImageIds(volumeId)` and `viewportService.scrollToIndex` handles volume viewports.
...
### MV-Phase 5: Tool audit + Contour Fill fix (In progress — opened 2026-05-03)
...
- ✅ **5.1** Tool inventory + audit. Walked every tool in [toolService.ts](src/renderer/lib/cornerstone/toolService.ts) and [tools/](src/renderer/lib/cornerstone/tools/) against §C3 / §C5 / §C6. Output: [`docs/multiviewport-phase5-audit.md`](docs/multiviewport-phase5-audit.md). Findings: lock + overlap layers are closed by architecture (3-layer guard for lock; active-segment-only writes for overlap); only Contour Fill is genuinely broken (Phase 5.2).
- ✅ **5.2** Fix Contour Fill (`LabelMapEditWithContourTool`). Root cause: the underlying Cornerstone tool's `checkContourSegmentation` relies on `TOOLGROUP_VIEWPORT_ADDED` firing **after** its `initializeListeners` registers — but `rebuildToolGroup` adds viewports first, then runs `applyBindings` → `setToolActive` → `initializeListeners`. The Contour rep is never attached, so `addContourSegmentationAnnotation` runs against an undefined `representationData.Contour`. Fix: in [`toolService.setActiveTool`](src/renderer/lib/cornerstone/toolService.ts#L700), call `segmentationService.ensureContourRepresentation(viewportId, segId)` before `rebuildToolGroup` for `ToolName.LabelmapEditWithContour` (both auto-create and existing-seg paths). Pinned by [`toolService.test.ts`](src/renderer/lib/cornerstone/__tests__/toolService.test.ts) "Phase 5.2: ensures a Contour representation exists when activating LabelmapEditWithContour" + E2E [`21-contour-fill-rasterize.e2e.ts`](e2e/specs/21-contour-fill-rasterize.e2e.ts) (flag-on + flag-off).
- ✅ **5.3** Active-segment lock enforcement audit (§C5). Closed by architecture: every editing tool routes through the same 3-layer guard (panel-disable / activation-refusal / pointerdown-block — see [audit doc §"Lock-enforcement architecture"](docs/multiviewport-phase5-audit.md)). `SafePaintFillTool` adds a fourth (tool-internal) layer, pinned by the new G21-backstop test in [`SafePaintFillTool.test.ts`](src/renderer/lib/cornerstone/tools/SafePaintFillTool.test.ts). G21 already pinned at the `RegionSegment` button + pointerdown layer by [`20-g21-region-segment-audit.e2e.ts`](e2e/specs/20-g21-region-segment-audit.e2e.ts). No additional per-tool fixes required.
- ✅ **5.4** Overlap policy enforcement (§C6). Closed by architecture: Cornerstone's brush-strategy / scissors / region-segment / contour-fill tools all write only the active segment; `SafePaintFillTool` skips voxels whose `oldValue === activeSegmentIndex` (no self-overwrite) and refuses fills whose seed is in a locked segment ([SafePaintFillTool.ts:171](src/renderer/lib/cornerstone/tools/SafePaintFillTool.ts#L171)). No additional per-tool fixes required. See [audit doc](docs/multiviewport-phase5-audit.md) for the per-tool table.
- ⏸️ **5.5** Per-contour canvas-side auto-marker (deferred from Phase 4 §B5). Decision after audit: defer. The row-level `~` glyph + Phase 4.8 `▶` step-through review button are sufficient for the §B5 transient-marker requirement at the row layer; adding a canvas-side rendering hook would touch Cornerstone's contour-rendering style internals (no public per-annotation style API today) and the row-level surface has not generated user-facing complaints during in-process QA. Re-open if user feedback elevates it.
- ✅ **5.6** Service-integration / E2E coverage. See acceptance signal coverage matrix below.
...
|---|---|---|---|---|
| **16** | 3D paint-fill on axial appears resampled on sagittal MPR; one undo reverts the entire fill as one entry | Phase 5.4 + `SafePaintFillTool` memo plumbing | Positive case + single-undo invariant: E2E [`23-g16-paint-fill-positive.e2e.ts`](e2e/specs/23-g16-paint-fill-positive.e2e.ts) brushes a closed square boundary, paint-fills the interior, asserts `afterFill > boundaryBaseline` (flood landed) AND `afterUndo === boundaryBaseline` exactly (one Ctrl+Z reverts only the fill memo, not the four brush memos). Unit-pinned by [`SafePaintFillTool.test.ts`](src/renderer/lib/cornerstone/tools/SafePaintFillTool.test.ts) "G16: a single fill records all voxel changes in one memo". Cross-MPR resampling follows from §C1 (G1 + G3 already pinned). | ✅ |
| **21** | Region-segment / smart brush fills connected voxels within intensity tolerance; locked segment blocks at gesture-start with hint | Phase 5.3 + Phase 5.4 + `RegionSegmentTool` / `RegionSegmentPlusTool` lock-block + `ct-axial-anatomy` fixture | Lock-block half: [`20-g21-region-segment-audit.e2e.ts`](e2e/specs/20-g21-region-segment-audit.e2e.ts) + `SafePaintFillTool` G21-backstop test. Positive half: [`22-g21-region-segment-positive.e2e.ts`](e2e/specs/22-g21-region-segment-positive.e2e.ts) drives Region Segment on the new [`ct-axial-anatomy`](e2e/fixtures/dicom/ct-axial-anatomy/) fixture (soft-tissue ellipsoid + bone insert + Gaussian noise; smooth gradient transitions); asserts non-zero painted voxels AND bounded grow (< 75% of slice). | ✅ |
| **§C3 Contour Fill (no G-signal)** | Closed contour rasterizes into active segment; only active segment receives voxels | Phase 5.2 | E2E [`21-contour-fill-rasterize.e2e.ts`](e2e/specs/21-contour-fill-rasterize.e2e.ts) (flag-on + flag-off): drives the production Add seg → Contour Fill → freehand stroke that closes → asserts non-zero PixelData voxels via `exportSegmentationToBase64`. Fails without the Phase 5.2 `ensureContourRepresentation` pre-flight. Plus unit coverage of the activation pre-flight in [`toolService.test.ts`](src/renderer/lib/cornerstone/__tests__/toolService.test.ts). | ✅ |
...
**Status (2026-05-03)**: Phase 5 closure gate satisfied. 5.1 audit doc landed at [`docs/multiviewport-phase5-audit.md`](docs/multiviewport-phase5-audit.md). 5.2 Contour Fill fix landed via [`toolService.setActiveTool`](src/renderer/lib/cornerstone/toolService.ts#L700) `ensureContourRepresentation` pre-flight; pinned by toolService unit test + E2E [`21-contour-fill-rasterize.e2e.ts`](e2e/specs/21-contour-fill-rasterize.e2e.ts). 5.3 / 5.4 closed by architecture (no per-tool fixes needed beyond `SafePaintFillTool`'s existing wrapper checks). 5.5 deferred (row-level surface sufficient). 5.6 evidence wired across G16, G21, §C3 Contour Fill rows — all three closed:
- **G16** (Paint Fill positive + single-undo): [`23-g16-paint-fill-positive.e2e.ts`](e2e/specs/23-g16-paint-fill-positive.e2e.ts) + [`SafePaintFillTool.test.ts`](src/renderer/lib/cornerstone/tools/SafePaintFillTool.test.ts) unit invariant.
- **G21** (Region Segment positive + lock-block): [`22-g21-region-segment-positive.e2e.ts`](e2e/specs/22-g21-region-segment-positive.e2e.ts) + [`20-g21-region-segment-audit.e2e.ts`](e2e/specs/20-g21-region-segment-audit.e2e.ts).
- **§C3 Contour Fill**: [`21-contour-fill-rasterize.e2e.ts`](e2e/specs/21-contour-fill-rasterize.e2e.ts).

New `ct-axial-anatomy` fixture (generator: [`scripts/generate-synthetic-fixture-ct-anatomy.mjs`](scripts/generate-synthetic-fixture-ct-anatomy.mjs)) provides realistic intensity statistics for intensity-aware tools and serves as the substrate for the G16 + G21 positive cases. Test suite at 1275 passing (was 1272 at end of Phase 4).

### MV-Phase 6: Legacy removal + flag flip (Complete — opened 2026-05-03, closed 2026-05-03)
...
- ✅ **6.1** Pre-flight regression matrix. Vitest baseline at 1275 passing on the Phase 5 closure point; Playwright spec set 01–23 covers every G-signal closed in Phases 1–5. Re-runs scheduled per sub-phase.
- ✅ **6.2** Remove legacy panel mounts. Realized via Stage 2B (option 2B from the gap analysis) — see Stage 2B sub-tasks below. **All four sub-tasks landed 2026-05-03.** `SegmentationPanel.tsx` (2,054 LOC), `AnnotationListPanel.tsx` (95 LOC), and four panel-internal helpers (`DeleteConfirmDialog`, `ExistingSaveDialog`, `PanelToast`, `SavingOverlay`) deleted; `ContainerListPanel` is the sole annotations side-panel surface.
- ✅ **6.3** Remove legacy viewport branches. `CornerstoneViewport` renamed to `StackViewport` (option 3A from the gap analysis); `Viewport.tsx` is now a 2-way switch between `StackViewport` (stack mode for non-volumetric data per `stackEligibility.ts` — US/XA/RF/NM/DX/CR/MG, multi-frame cine, single-image) and `VolumeViewport` (volume mode for everything else). **Landed 2026-05-03.** Pure rename: file `CornerstoneViewport.{tsx,test.tsx}` → `StackViewport.{tsx,test.tsx}` via `git mv`; identifier and `data-testid` (`cornerstone-viewport-canvas` → `stack-viewport-canvas`, plus `-status`, `-error`, `-pending`, `-` variants) updated across 31 source/test/e2e files. No behavior change. 1248 vitest tests still passing post-rename.
- ✅ **6.4** Remove `enterMPR` / `MPRViewportGrid` legacy path. `viewportLayoutService.applyPreset('mpr-2x2')` is the only MPR entry. **Landed 2026-05-03.**
  - **Deleted**: `MPRViewportGrid.tsx` + test (250 LOC), `MPRViewport.tsx` + test (281 LOC), `mprService.ts` + test (197 LOC), `mprToolService.ts` + test (110 LOC), `OrientedViewport.tsx` + test (228 LOC — pulled forward from 6.3 since its only `mprService` dependency vanished). Total ~1,500 LOC source + ~700 LOC tests deleted.
  - **viewerStore**: removed `mprActive`, `mprVolumeId`, `mprSourcePanelId`, `mprPriorState`, `mprViewports`, `mprVolumeProgress` state + `enterMPR` / `exitMPR` / `_updateMPRSlice` / `_updateMPRVolumeProgress` actions. Imports of `mprToolService`, `volumeService`, `generateVolumeId`, `MPRViewportState`, `VolumeLoadProgress` dropped.
  - **App.tsx**: deleted `enterMprForPanel` callback, the flag-off branch in `handleToggleMPR` (the function now unconditionally takes the mpr-2x2 preset path), the `mprActive` check in the openInMpr scan-load branch, the `mprSourceImageIds` derivation + ViewerPage prop, the `volumeService` import.
  - **ViewerPage.tsx**: removed the `mprActive ? <MPRViewportGrid /> : <ViewportGrid />` conditional (always `<ViewportGrid />` now), the `mprActive` / `mprVolumeId` selectors, the `mprSourceImageIds` prop. The `!mprActive &&` guards around the side panels deleted (panels show on every layout including MPR now).
  - **ViewportGrid.tsx**: removed the `OrientedViewport` import + the `multiViewportEnabled ? <Viewport orientation={...} /> : <OrientedViewport />` conditional. Always routes through `Viewport` with the orientation prop.
  - **ScrollSlider**: removed `mprService.scrollToIndex` branch; uses `viewportService.scrollToIndex` for both stack and volume viewports (volume viewports populate `viewports[pid].totalImages` via `_updateImageIndex` per Phase 4.8).
  - **hotkeyService**: removed `mprService` import, `mprActive` guard rails on layout switching + cine, `mpr_panel_*` slice-navigation branch, oriented-viewport branch. Slice nav unified through `viewports[pid]` + `viewportService.scrollToIndex`.
  - **Toolbar.tsx**: replaced `mprActive` viewerStore field (gone) with a derived check on `panelOrientationMap[panel_0..2]`. **Behavior change**: navigation tools (Crosshairs / W/L / Pan / Zoom), annotation tools, transform tools, and cine controls all stay enabled in MPR mode — the legacy "MPR is a separate mode that hides everything" UX gating is gone. The MPR toggle button still flips between Enter/Exit affordance based on the derived state. Layout dropdown enabled in MPR mode (no more `disabled` prop on `LayoutDropdown`).
  - **crosshairGeometry / crosshairSyncService**: `mprService` lookups switched to `viewportService.getVolumeViewport(panelId)` (volume + stack viewports both live in the shared rendering engine). `mprService.scrollToIndex` callsite in crosshairSync replaced with `viewportService.scrollToIndex`.
  - **viewportLayoutService**: docstring updated to drop "Replaces the parallel MPRViewportGrid ↔ ViewportGrid switch" framing — that's done.
  - **issueReport**: dropped `MPR active: ${viewer.mprActive}` line from the diagnostic snapshot.
  - **e2e bridge** (`installRendererE2eHooks.ts`): `toggleMpr` no longer branches on flag — always uses the mpr-2x2 preset path. `volumeService` import dropped.
  - **shared types**: deleted `MPRViewportState`, `mprPanelId()`, `MPR_PANELS`, `VolumeLoadProgress`. `MPRPlane` and `ViewportOrientation` retained (still used by ViewportOverlay).
  - **Tests updated**: `viewerStore.test.ts` (deleted enterMPR/exitMPR + _updateMPR* assertions), `hotkeyService.test.ts` (deleted MPR guard-rail + MPR-slice-nav tests, kept stack slice nav), `ViewerPage.hotkeys.test.tsx` (dropped MPRViewportGrid/mprService mocks), `ScrollSlider.test.tsx` (rewrote oriented-viewport assertion against unified scrollToIndex), `ViewportGrid.test.tsx` (replaced CornerstoneViewport+OrientedViewport stubs with a single Viewport stub), `Toolbar.test.tsx` (rewrote MPR-mode test to use panelOrientationMap derivation, asserts navigation tools stay available), `App.test.tsx` (rewrote MPR toggle test to assert orientation-map side effect rather than volumeService.create call), `crosshairGeometry.test.ts` + `crosshairSyncService.test.ts` (dropped mprService mocks), `issueReport.test.ts` (dropped mprActive setter).
  - **Spec 10**: deleted the flag-off `test.fixme` variant ("legacy stack + legacy MPRViewportGrid"). The flag-on (volume + viewportLayoutService preset) variant is the survivor and was already passing. `test.fixme` count drops by 1.
  - **Test count**: 1248 vitest passing (was 1275; 27 tests were exercising deleted MPR code). Pre-existing tsc error count went from 69 → 67 (no new errors introduced; the 2 reduction is from tests covering deleted code).
- ✅ **6.5** CrosshairsTool ↔ WindowLevel binding flip — **RETIRED with rationale**. The flip was scoped on the assumption "now that stack viewports are gone", but per the Phase 6.3 gap analysis (option 3A), stack mode survives for non-volumetric data (US/XA/RF/NM/DX/CR/MG, multi-frame cine, single-image). The custom crosshair handlers in [`crosshairGeometry.ts`](src/renderer/lib/cornerstone/crosshairGeometry.ts) + [`crosshairSyncService.ts`](src/renderer/lib/cornerstone/crosshairSyncService.ts) work for both stack and volume viewports; flipping to Cornerstone's CrosshairsTool would break crosshair sync on the surviving stack-mode panels. The custom-handler design serves all viewport types and stays. Documented in [`toolService.ts`](src/renderer/lib/cornerstone/toolService.ts#L114) at the `[ToolName.Crosshairs]: WindowLevelTool.toolName` site. **Decided 2026-05-03.**
- ✅ **6.6** Delete the `multiViewport.enabled` flag itself. **Landed 2026-05-03.**
  - **Type definition**: `MultiViewportPreferences.enabled` removed from [`src/shared/types/preferences.ts`](src/shared/types/preferences.ts); only `crossSeriesRendering` survives. `DEFAULT_MULTIVIEWPORT_PREFERENCES` no longer carries `enabled`.
  - **Store**: `setMultiViewportEnabled` action removed from [`preferencesStore.ts`](src/renderer/stores/preferencesStore.ts). The persistence-merge layer silently ignores any incoming `enabled` key from old persisted state (no migration needed; field just doesn't surface on the type).
  - **Production gates removed**: every `usePreferencesStore.getState().preferences.multiViewport.enabled` callsite was `false` → bail-early under the legacy world; under Phase 6.4+ they are all unconditionally `true`. Deleted from [`autoSave.ts`](src/renderer/lib/cornerstone/segmentationService/autoSave.ts) (per-container `transport.notifyDirty` now unconditional), [`historyMemo.ts`](src/renderer/lib/cornerstone/segmentationService/historyMemo.ts) (`autoGeneratedMemoBuffer` divert is unconditional, so interpolation passes always collapse to one undo entry per design §B5), [`contourHoverSync.ts`](src/renderer/lib/cornerstone/contourHoverSync.ts) (mousemove always schedules the hit-test rAF), [`toolService.ts`](src/renderer/lib/cornerstone/toolService.ts) (B3 drawing-routing block always runs at pointerdown), and 5 sites in [`segmentationService.ts`](src/renderer/lib/cornerstone/segmentationService.ts) (D9 styling, hover dispatch, container-bridge styling re-apply).
...
The original 6.2 plan ("Remove legacy panel mounts") presumed `ContainerListPanel` had feature parity with `SegmentationPanel`. The gap analysis identified four user-facing concerns that lived only in `SegmentationPanel`: Add SEG / Add structure dialogs, tool palette + brush-size slider, Save Local + Upload to XNAT flow. Stage 2B splits these to where they conceptually belong:

- ✅ **2B.1** Move "Add SEG" / "Add structure" buttons to toolbar. New [`AddAnnotationButtons.tsx`](src/renderer/components/viewer/AddAnnotationButtons.tsx) (~140 LOC) owns its own naming dialog and reads activeViewportId / sourceImageIds / panelXnatContextMap / panelScanMap from the existing stores — no new prop wiring. Mounted in `Toolbar.tsx`'s annotation-tools collapsible group alongside `SegmentationPanelToggle` and `AnnotationToolDropdown`. Same data-testids preserved (`add-segmentation-btn`, `add-structure-btn`) so the e2e specs still find them. The corresponding buttons in `SegmentationPanel.tsx`'s header deleted (Save All survives until 2B.2). 3 new unit tests in [`AddAnnotationButtons.test.tsx`](src/renderer/components/viewer/AddAnnotationButtons.test.tsx) cover the disable-without-images, SEG-create-with-default-segment, and RTSTRUCT-create branches; corresponding tests in `SegmentationPanel.test.tsx` removed (the moved functionality is no longer panel-internal). **Landed 2026-05-03.**
- ✅ **2B.2** Move "Upload to XNAT" into ContainerListPanel's per-container ⋯ menu. **Landed 2026-05-03.**
  - **New service**: [`xnatUploadService.ts`](src/renderer/lib/cornerstone/xnatUploadService.ts) (~310 LOC) extracts the full `handleUploadXnat` flow from the legacy SegmentationPanel: panel-context lookup → exportable-content guard → existing-save prompt (overwrite vs. cancel) → export base64 → choose `overwriteDicomSeg` / `overwriteDicomRtStruct` / `uploadDicomSeg` / `uploadDicomRtStruct` → record xnatOrigin → refresh session scans + UID associations → cleanup local backups + legacy temp files. Every UI concern (toast, dialog) injected via `wireXnatUpload({ getPanelXnatContext, getSourceScanId, exportToBase64, promptExisting, notify })` so the service stays panel-free.
  - **Default wiring** (in [`segmentationService.initialize()`](src/renderer/lib/cornerstone/segmentationService.ts)): `notify` routes to `showAlertDialog`; `promptExisting` routes to `showConfirmDialog` (overwrite vs. cancel — the legacy ExistingSaveDialog's create-new-with-labeled-scan path is a future enhancement). `exportToBase64` defaults to the standard SEG / RTSTRUCT exporter.
  - **containerActions.uploadContainerToXnat(containerId)**: high-level entry that resolves the container's `csSegmentationId` + `kind`, dispatches to the service, returns `'saved' | 'cancelled' | 'failed'`. POI containers refused (not yet supported).
  - **ContainerListPanel ⋯ menu**: new "Upload to XNAT…" entry alongside Save / Revert / Export. data-testid `container-menu-upload-xnat:{id}`.
  - **Tests**: 4 new unit tests in [`xnatUploadService.test.ts`](src/renderer/lib/cornerstone/xnatUploadService.test.ts) cover the four most-load-bearing branches: no-panel-context fail, no-content fail, prompt-cancelled, fresh upload as new SEG. The full multi-electronAPI happy path is covered by the existing E2E suite when it re-runs.
  - **Dynamic imports**: `dicomwebLoader`, `rtStructService`, `backupService`, and `useSessionDerivedIndexStore` use `await import('...')` rather than top-level imports because their static graph pulls in `@cornerstonejs/dicom-image-loader/wadouri` (web-worker module that fails to parse in JSDOM). Keeps the `containerActions.test.ts` + `xnatUploadService.test.ts` units green without mocking the entire Cornerstone web-worker stack.
- ✅ **2B.3** Move brush-size slider into `SegmentationToolDropdown`. The slider lives in the dropdown's open panel above the tool tiles, visible only when a brush-style tool (Brush / Eraser / ThresholdBrush) is the active primary. Reads/writes go through `useSegmentationStore.brushSize` + `segmentationService.setBrushSize` — same store + service the panel-internal slider used. New `BrushSizeSlider` helper at the bottom of [`SegmentationToolDropdown.tsx`](src/renderer/components/viewer/SegmentationToolDropdown.tsx). Slider survives in `SegmentationPanel.tsx` for now (deletes when 2B.4 deletes the whole panel). **Landed 2026-05-03.**
- ✅ **2B.4** Delete `SegmentationPanel.tsx` + `AnnotationListPanel.tsx`. **Landed 2026-05-03.**
  - **Source files deleted** (~2,400 LOC + tests): `SegmentationPanel.tsx` (2,054 LOC), `SegmentationPanel.test.tsx`, `AnnotationListPanel.tsx` (95 LOC), `AnnotationListPanel.test.tsx`, plus four panel-internal helpers — `DeleteConfirmDialog.tsx`, `ExistingSaveDialog.tsx`, `PanelToast.tsx`, `SavingOverlay.tsx`.
  - **`useAnnotationStore`**: dropped `showPanel` + `togglePanel`. Annotation summaries (`annotations`, `selectedUID`, `_sync`, `select`) survive — annotationService still drives them.
  - **`useSegmentationStore.activeSegTool` survives**: still load-bearing for [`SegmentationManager.userSelectedSegmentation`](src/renderer/lib/segmentation/SegmentationManager.ts) which uses it to deactivate the active tool when the user switches to a locked or type-incompatible segmentation. Cosmetic but real signal.
  - **Hotkey rebind**: `panel.toggleAnnotations` (legacy "o" key, was `useAnnotationStore.togglePanel`) now collapses with `panel.toggleSegmentation` to call `useSegmentationStore.togglePanel`. Both hotkeys toggle the (sole-surviving) ContainerListPanel mount.
  - **ViewerPage.tsx**: rewritten as a minimal shell — drops `SegmentationPanel` / `AnnotationListPanel` imports + mounts; `ContainerListPanel` mounts unconditionally on `useSegmentationStore.showPanel` (preserved name, repurposed: now means "show the annotations panel" since there's only one).
  - **`Toolbar.tsx` SegmentationPanelToggle survives**: still toggles `useSegmentationStore.showPanel`, which now controls ContainerListPanel visibility. No user-visible behavior change beyond which panel mounts.

#### Phase 6.4 closure note

The 6.4 deletion went deeper than the original plan suggested because `OrientedViewport` (originally scoped for 6.3) was logically unreachable once the legacy MPR path was gone — its only `mprService` dependency had vanished. Pulling it forward into 6.4 saved a transient broken state. `CornerstoneViewport` survives untouched in 6.4; it's the surviving stack-mode renderer until 6.3 renames it to `StackViewport`.

The Toolbar UX change (navigation/annotation/cine tools no longer hidden in MPR mode) is intentional and aligned with the design's "MPR is just a layout preset" intent — the legacy gating assumed a separate tool group that no longer exists. The MPR toggle button still flips Enter/Exit so users can leave MPR mode.

Stage 2 (Gap A) decision: option **2B** — split SegmentationPanel's concerns to the toolbar (Add SEG / Add structure buttons, brush size slider) and ContainerListPanel ⋯ menu (Upload to XNAT). See the Stage 2B sub-tasks above for status.

#### Phase 6 closure (2026-05-03)

**All sub-tasks landed in a single session**:
- 6.1 pre-flight regression matrix
- 6.2 panel mounts removed (via Stage 2B.1 → 2B.4)
- 6.3 StackViewport rename
- 6.4 legacy MPR path removal
- 6.5 CrosshairsTool flip retired with rationale (custom handlers serve all viewport types)
- 6.6 `multiViewport.enabled` flag deleted
- 6.7 final regression sweep (vitest); Playwright re-run scheduled per the matrix below

**Test count timeline**:
- End of Phase 5: 1275 vitest passing
- After Phase 6.4 (MPR removal): 1248
- After 6.3 rename: 1248 (no impact)
- After 2B.1 + 2B.3: 1250 (3 new tests for AddAnnotationButtons; 1 SegmentationPanel test deleted)
- After 2B.2: 1254 (4 new tests for xnatUploadService)
- After 2B.4: 1248 (6 panel tests deleted with SegmentationPanel + AnnotationListPanel)
- After 6.6: **1240** (5 multiViewport-flag tests deleted; 2 contourHoverSync flag-off + 1 baseline adjustment; 1 spec missed)

**Code-size delta**: removed ~5,500 LOC of legacy production code + ~1,400 LOC of legacy tests; added ~600 LOC of replacement components (AddAnnotationButtons, xnatUploadService, ViewerPage rewrite, Viewport simplification) + ~300 LOC of new tests. Net: **~6,000 LOC removed**.

**Out-of-scope-for-this-session cosmetic cleanup** (future PR): 78 e2e-spec call sites to `setMultiViewportEnabled` / `multiViewportEnabled` survive in `e2e/specs/*.e2e.ts` and `e2e/helpers/fixture-load.ts`. They route through bridge no-op shims and are functionally inert. Cleanup is mechanical (delete the calls + drop the option from the helper signature) and best done as its own focused PR so the diff stays trivially reviewable.
...
|---|---|---|---|
| G1 | MV-Phase 1 | ✅ via [`13-g1-acceptance.e2e.ts`](e2e/specs/13-g1-acceptance.e2e.ts) | Re-run on legacy-removed branch (mpr-2x2 path is the survivor). |
| G2 | MV-Phase 1 | ✅ via [`12-g2-g8-acceptance.e2e.ts`](e2e/specs/12-g2-g8-acceptance.e2e.ts) | Re-run. |
...
| G5 | MV-Phase 3 | ✅ via [`g5-per-viewport-hide.test.ts`](src/renderer/lib/cornerstone/__tests__/g5-per-viewport-hide.test.ts) | Re-run; verify alt+click affordance still hits the per-viewport API on the surviving viewport implementation. |
...
| G7 | MV-Phase 1 / 2 | ✅ flag-off + flag-on via spec 09 | Re-run. Both variants pass after the e2eFixtureBridge XNAT-context synthesis (2026-05-03). |
| G8 | MV-Phase 3 | ✅ via [`12-g2-g8-acceptance.e2e.ts`](e2e/specs/12-g2-g8-acceptance.e2e.ts) | Re-run. |
| G9–G12 | MV-Phase 2 | ✅ via [`16-cross-series-acceptance.e2e.ts`](e2e/specs/16-cross-series-acceptance.e2e.ts) (G10/G11/G12) + [`11-fixture-cross-series.e2e.ts`](e2e/specs/11-fixture-cross-series.e2e.ts) (G9 classifier) | Re-run. G9 visual layer (dashed pixel rendering, hover tooltip) intentionally not part of the closure-gate evidence. |
| G13, G22 | MV-Phase 4 | ✅ via [`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts) | Re-run. |
| G14, G15 | MV-Phase 2 | ✅ via [`18-save-flow-acceptance.e2e.ts`](e2e/specs/18-save-flow-acceptance.e2e.ts) | Re-run. |
| G16 | MV-Phase 5 | ✅ via [`23-g16-paint-fill-positive.e2e.ts`](e2e/specs/23-g16-paint-fill-positive.e2e.ts) (positive + single-undo E2E) + [`SafePaintFillTool.test.ts`](src/renderer/lib/cornerstone/tools/SafePaintFillTool.test.ts) (unit invariant) | Re-run. |
| G21 | MV-Phase 5 | ✅ both halves: lock-block via [`20-g21-region-segment-audit.e2e.ts`](e2e/specs/20-g21-region-segment-audit.e2e.ts) + positive via [`22-g21-region-segment-positive.e2e.ts`](e2e/specs/22-g21-region-segment-positive.e2e.ts) on `ct-axial-anatomy` | Re-run. |
| G17 | MV-Phase 3 | ✅ via [`g17-active-member-append.test.ts`](src/renderer/lib/cornerstone/__tests__/g17-active-member-append.test.ts) | Re-run. |
| G18, G19 | MV-Phase 3 | ✅ via [`17-rtstruct-roundtrip.e2e.ts`](e2e/specs/17-rtstruct-roundtrip.e2e.ts) | Re-run. |
