...
- ⏳ **3.8c** Inline ROI type editor (clickable badge → dropdown) and E2E for signals 18 (ROI type round-trip), 19 (approval persistence), 20 (visibility mode round-trip is session-only — already covered), 22 (provenance round-trip).

**Status (2026-05-03, in progress)**: data layer + UI shell + visibility cycling + selection model + row hover + member CRUD + action menu + filter/search + A2c opt-in + sort + approval UI + ROI type / provenance landed (3.1 → 3.8b). Test suite at 1002 passing. Legacy panels still mount under flag-off; ContainerListPanel mounts alongside under flag-on for verification. Outstanding sub-phases: 3.8c (inline ROI type editor + E2E for signals 18/19/22), and the bidirectional canvas ↔ row hover sync (Phase 3.5b reciprocal — staged as its own follow-on because it requires Cornerstone tool internals).
