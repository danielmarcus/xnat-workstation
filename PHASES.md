...
- **Slices:** 1 FoR-eligibility service (pure) → 2 FoR-gated attach + non-native style (D9) → 3 gesture-start blocking → 4 per-container undo → 5 queue/debounced autosave → 6 copy/paste finish. **All six committed; 603 unit tests green; tsc 60 baseline; build + lint clean.**
  ...
  - **🔒 Still blocked / pending:**
    - **(a') eligibility-attach into the live SEG flow** — route `segmentationService.addToViewport` (and `createUnifiedLabelmapSegmentation`) through `attachLabelmapWithEligibility` so cross-series panels get the non-native style / hide / different-FoR skip, WITHOUT breaking the MPR same-volume attach (native → attach normally). Then signals 9-dimming/10/11 (fixme in spec 30) go green. Substantial wiring with MPR-regression risk.
