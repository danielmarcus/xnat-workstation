...
- ⏳ **Local DICOM fixtures** (e2e/fixtures/dicom/) — *Item 4*: harness landed in Phase 1 (loader + 10 unit tests). Helper-level pipeline acceptance landed (`e2e/specs/11-fixture-cross-series.e2e.ts`): discovers a fixture, parses files with dcmjs, validates the metadata-shape claim; skips cleanly when no fixture is on disk. **Renamed** the design's `breath-hold-pair` slot to `sameforuid-different-acquisition` since the A2c heuristic keys on metadata (shared FoR + differing `AcquisitionNumber`), not anatomy displacement; same fixture covers the design's `4dct-phases` scenario. **Storage**: Git LFS, configured via `.gitattributes` at the repo root. **Populated** (both fixtures, spec 2 / 2 pass): (1) `sameforuid-different-acquisition/` — 1.1 MB, 32 synthetic CT slices, two series sharing FoR with `AcquisitionNumber` 1 / 5; generator at `scripts/generate-synthetic-fixture-sameforuid.mjs`. (2) `mr-t1-t2-sameexam/` — 6.9 MB, 8 middle slices each of AX T1 + AXIAL T2 from TCIA `ACRIN-NSCLC-FDG-PET-099` (de-identified at source); both series share one `FrameOfReferenceUID`. **Outstanding**: (a) renderer hook to mount local-file paths into a viewport so the spec can drive canvas-level assertions for signals 9 (A2b dashed stroke) and 10 (A2c off-by-default); (b) the other six fixture sets remain pending and will be picked up as Phase 2 work demands them.
