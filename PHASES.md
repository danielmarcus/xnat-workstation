...
- ⏳ **3.8e** (proposed follow-up) Service-layer edit-lock enforcement when approved. Currently the action menu hides at the UI layer but `containerService.setRoiType` / `setMemberVisibility` / etc. still mutate. The service should throw or no-op on approved containers.
- ⏳ **3.8f** (deferred) Playwright E2E for the same signals once cross-series + DICOM SEG / RTSTRUCT save-load fixtures land.

**Status (2026-05-03)**: data layer + UI shell + visibility cycling + selection model + row hover + row-to-canvas hover sync + member CRUD + action menu + filter/search + A2c opt-in + sort + approval UI + ROI type / provenance + inline ROI type editor + signal 18/19 service-integration landed (3.1 → 3.5c-row + 3.6 + 3.7 + 3.8a–d). Test suite at 1029 passing. Legacy panels still mount under flag-off; ContainerListPanel mounts alongside under flag-on for verification. Outstanding sub-phases: 3.5c-canvas (canvas → row reciprocal hover sync — Cornerstone hit-testing), 3.8e (service-layer edit-lock when approved), 3.8f (Playwright E2E once fixtures land), and signal 22 (provenance round-trip via interpolation — Phase 4 territory).
