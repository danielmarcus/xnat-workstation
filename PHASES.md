...
- ⏳ **3.6** Member CRUD wiring through containerService (createMember / deleteMember / renameMember / recolorMember). Per-member action menu (D7.6).
...
**Status (2026-05-02, in progress)**: data layer + UI shell + visibility cycling + selection model + row hover landed (3.1 → 3.5b). Test suite at 927 passing. Legacy panels still mount under flag-off; ContainerListPanel mounts alongside under flag-on for verification. Outstanding sub-phases: 3.6 (member CRUD + actions menu), 3.7 (A2c opt-in + filter/search/sort + empty/loading states), 3.8 (approval UI + ROI type + provenance + E2E for signals 18/19/20/22), and the bidirectional canvas ↔ row hover sync (Phase 3.5b reciprocal — staged as its own follow-on because it requires Cornerstone tool internals).
