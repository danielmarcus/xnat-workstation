...
> - [x] **C6** 4D / multi-volume functional images render wrong off-axis. ✅ `1599428` — root cause: the dynamic volume loader was never registered, so the static loader packed all time points into one volume (corrupt geometry). Now init registers both loaders + `volumeService.acquire` routes 4D (getDynamicVolumeInfo) → dynamic scheme → StreamingDynamicImageVolume (correct per-time-point geometry). Verified: volumeService.test routing (4D→dynamic/3D→static/throw→static) + 3D E2Es unchanged. ⚠️ RENDERING fix only (current time point, all planes); on-real-functional-series confirmation pending (no 4D fixture offline — diagnostic: console "[volumeService] 4D / multi-volume series → dynamic volume"). **Follow-up: a time-point scrubber to navigate volumes.**
