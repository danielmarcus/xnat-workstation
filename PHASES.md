...
- ⏳ **3.8d** Playwright E2E for signals 18 (ROI type round-trip), 19 (approval persistence). Both signals require the cross-series + DICOM SEG / RTSTRUCT save-load fixtures Phase 1 deferred. Signal 20 (visibility mode) is session-only per §D7.10 — verified at the service-integration layer in Phase 2.9. Signal 22 (provenance round-trip) needs an interpolation flow in tests; defer until interpolation Phase 4 or fixture work.

**Status (2026-05-03, end of in-session work)**: data layer + UI shell + visibility cycling + selection model + row hover + member CRUD + action menu + filter/search + A2c opt-in + sort + approval UI + ROI type / provenance + inline ROI type editor landed (3.1 → 3.8c). Test suite at 1008 passing. Legacy panels still mount under flag-off; ContainerListPanel mounts alongside under flag-on for verification. Outstanding sub-phases: 3.8d (Playwright E2E for signals 18/19/22 — fixture-blocked) and the bidirectional canvas ↔ row hover sync (Phase 3.5b reciprocal — staged as its own follow-on because it requires Cornerstone tool internals).
