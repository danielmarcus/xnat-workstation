...
- **P1.8 — Wire UI → unified, flip default, delete legacy.**
  ...
  - **P1.8e (remaining) — fold in the carried-over `segmentationService` decomposition.** The deferred pure-extraction of `segmentationService.ts` (~5,600 lines) into `contourTools`/`undo`/`visibility`/`representation`/`lifecycle` modules (re-export to keep the public API stable). Distinct, behavior-preserving refactor; verifiable now against the rebuilt viewport. Can be its own focused pass.
