...
| G16 | 3D paint-fill on axial appears resampled on sagittal MPR; one undo reverts entire fill | MV-Phase 5 | ⏳ scheduled (mechanism partially audited 2026-05-03) | `SafePaintFillTool` is wired and exposed via `SegmentationPanel`'s "Paint Fill" button. The "fill a connected region" acceptance requires the user to first establish a closed boundary (brush + paint-inside or contour + paint-inside); naive "click on empty volume" is correctly refused by `SafePaintFillTool`'s anti-accidental-full-slice safeguards (see `tools/SafePaintFillTool.ts:197-218`). Full G16 E2E needs the multi-step boundary setup — substantive Phase 5 spec work, not Cornerstone capability. The single-undo-reverts-entire-fill invariant follows from `BrushTool.doneEditMemo` which G7's spec already pins. |
...
| G21 | Region-segment / smart brush; lock blocks at gesture-start | MV-Phase 5 | ✅ pinned (lock-block layer) | E2E ([`20-g21-region-segment-audit.e2e.ts`](e2e/specs/20-g21-region-segment-audit.e2e.ts), 2026-05-03) — verifies the lock-block invariant is doubly enforced: (1) `SegmentationPanel` disables Region Segment + Brush buttons when the active segment is locked; (2) the pointerdown lock guard at `toolService.ts:158-189` prevents voxel writes even if the button is bypassed. The "Region Segment fills connected voxels within intensity tolerance" positive case requires real CT data with anatomy (synthetic CT's seed-rejection safeguards refuse the click); positive case still ⏳ until a real-CT fixture lands. |
...
| G16 | MV-Phase 5 | partial — `SafePaintFillTool` wired; full E2E needs boundary-setup spec | Re-run Phase 5 evidence. |
| G21 | MV-Phase 5 | ✅ lock-block layer via [`20-g21-region-segment-audit.e2e.ts`](e2e/specs/20-g21-region-segment-audit.e2e.ts); positive case ⏳ on real-CT fixture | Re-run. |
