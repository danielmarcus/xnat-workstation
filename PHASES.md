...
- **P1.7 — Turn signals 1/3/6/7 green (real-Cornerstone E2E, visual).** Decision 2026-06-08 (user): the four signals all require annotation/segmentation **editing** that pure viewport-unification excludes, so the minimal editing/undo/dirty-save needed to satisfy them is **pulled forward into Phase 1** (rather than re-scoping). Reuse the existing `segmentationService` machinery (brush/contour/SEG/undo via global `DefaultHistoryMemo`/dirty flag/`exportToDicomSeg`) — it uses **viewport-focused** Cornerstone APIs + a **global** history, so it wires onto the unified tool group + viewports with minimal new code. Ordered by capability dependency (each red→green, commit per slice):
  - **P1.7a — drawing/segmentation tools into the unified tool group + active-tool control.** Add `LengthTool`, `PlanarFreehandContourSegmentationTool`, `BrushTool` (+ scissors as needed) to `xnatToolGroup_unified`; add `unifiedToolService.setActiveTool(toolName)` (active tool → Primary; Crosshairs demoted to Passive so reference lines still render). E2E hook + assert active tool.
  - **P1.7b — signal 1: freehand contour on axial → live on sagittal/coronal.** MPR-2×2 of one CT; activate FreehandContour; draw a contour on axial via a real mouse gesture; assert it renders on the sagittal + coronal panels (shared volume). Visual/snapshot.
  - **P1.7c — signal 3: brush SEG on stack → MPR resampled.** Add a `stack+mpr` layout preset (one stack panel + MPR volume panels of the same volume); create a labelmap SEG on the shared volume + add representation to the unified viewports; brush-paint on the stack panel via real gesture; assert painted voxels appear resampled on the MPR panels (PolySeg).
  - **P1.7d — signal 7: undo a brush stroke from a closed panel.** Brush on a panel, switch layout so that panel is destroyed, undo (delegates to the global `DefaultHistoryMemo`), assert the stroke is undone. Wire the `undoService` skeleton to the real undo.
  - **P1.7e — signal 6: layout swap loses nothing + single dirty flag + save.** Create a structure, switch layouts 2×2→1×1→MPR→2×2 rapidly, assert no loss/duplicate/stale + single `hasUnsavedChanges` flag; `exportToDicomSeg` once produces a SEG that parses + validates offline (Rows/Columns/segments consistent).
