...
- **Slices:** 1 FoR-eligibility service (pure) → 2 FoR-gated attach + non-native style (D9) → 3 gesture-start blocking → 4 per-container undo → 5 queue/debounced autosave → 6 copy/paste finish. **All six committed; 603 unit tests green; tsc 60 baseline; build + lint clean.**
  ...
  - **🔒 Still pending:**
    - **signal 10 (A2c displacement-hide)** — `classifyEligibility` still gets no `bulkDisplacementMm` (live two-volume scalar-data read not wired), so a displaced sibling classifies cross-series-SHOW not -HIDE. `fixme` in spec 30. Needs the volume-cache scalar read fed into the attach classification.
    - **signal 9b (D9 non-native styling, pixel)** — the dimmed style IS applied on cross-series attach (unit-verified, Slice 2); a pixel-level "dimmer than native" assertion needs a tolerance diff (the two panels show different base series). `fixme` in spec 30.
    - **signal 14 live autosave** — needs the XNAT transport workstream.
    - **(e) A2c displacement-hide live volume read** — the classifier + `bulkDisplacement` estimator are unit-tested (Slice 1); wiring the live two-volume scalar-data read is feasible but its acceptance (signal 10, breath-hold) also needs the two-panel harness.
    - **(d) signal 14 live autosave** — needs the real per-container **XNAT transport** (transport workstream; WADO-RS not implemented). `saveQueue` mechanism is verified (Slice 5); `backupService` autosave stays live until transport lands.
    - **(f) signal 23 live** — voxel GPU copy/paste + Ctrl-C/Ctrl-V routing + the draw→copy→scroll→paste pixel-diff E2E. The NN-resample math is verified (Slice 6); the live path needs the harness + the voxel/clipboard wiring.
    - **Unblock path:** build the two-panel multi-series Playwright pixel-diff harness (fixtures already exist) for 9/10/11/23, and the XNAT transport for 14. Both are substantial standalone efforts.
...
- **TR5 (mocked-XNAT approach, chosen 2026):**
  ...
  - **🔒 Remaining for the live round-trip:** extend the main-process IPC upload/overwrite handlers (`src/main/ipc/uploadHandlers.ts`) to RETURN a version token (etag/last-modified) so the production `XnatUploadApi` matches the adapter's contract; wrap `electronAPI.xnat` as the production `XnatUploadApi`; wire it via `setSaveTransport`; production edit path must drive `notifyContainerDirty` (gesture interceptor) + autosave-to-XNAT is explicit opt-in (default off — `backupService` local-fs stays live until then). RTSTRUCT/SR write variants.
  - **E2E:** install `mockXnatApi` in place of `electronAPI.xnat` to drive signal-14 autosave + signal-27 conflict end-to-end offline.

### XNAT-browser lifecycle track — sliced plan (signals 25/26, A13) (Not started)
> Auto-load annotations when a session/scan is selected in the XNAT Browser (transport B5; no manual load), preserve the panel across same-session scan navigation, and retain unsaved containers across session switches with an unsaved-work banner. Current state (scoped 2026): `autoLoadSegOnScanClick` defaults on; `App.loadSessionFromXnat` auto-loads at SESSION load; but single-scan clicks (`loadFromXnatScan`) don't auto-load, same-session navigation CLEARS the panel, and session-switch discards ALL containers (no dirty-retention, no banner). **Not offline-testable via the local-fixture harness — needs the XNAT-Browser flow (live/mocked session).**
- **L1 (buildable offline, pure logic)** — a `sessionLifecycle` decision module: given (currentSession, nextSession, loadedContainers w/ dirty flags), decide per container retain-in-memory / unload / keep-rendered. Unit-testable. Encodes A13: same-session navigate ⇒ preserve; session-switch ⇒ unload clean, RETAIN dirty.
- **L2 (integration)** — wire L1 into `App.loadFromXnatScan`/`loadSessionFromXnat` + `segmentationManager`: stop clearing the panel on same-session navigate (re-evaluate FoR-eligibility per A2 instead); extend auto-load to single-scan clicks.
- **L3 (UI)** — the unsaved-work banner (`N sessions with unsaved annotations · Review`) reading the retained-dirty map; restore on return.
- **L4 🔒 verification** — signals 25/26 E2E need the XNAT-Browser-driven load (live/mocked session) — not the offline local-fixture harness. (L1's logic is offline-unit-verifiable; the full lifecycle E2E is browser-gated.)
