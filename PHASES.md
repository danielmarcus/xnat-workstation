...
**Status (2026-05-03)**: data layer + UI shell + visibility cycling + selection model + bidirectional hover sync (row ↔ canvas, contour AND labelmap) + member CRUD + action menu + filter/search + A2c opt-in + sort + approval UI + ROI type / provenance + inline ROI type editor + signal 18/19 service-integration + service-layer edit-lock when approved landed (3.1 → 3.5c-canvas-labelmap + 3.6 + 3.7 + 3.8a–e). Test suite at 1114 passing. Legacy panels still mount under flag-off; ContainerListPanel mounts alongside under flag-on for verification. Phase 3 list-panel work is structurally complete. Only outstanding sub-phase: **3.8f** (Playwright E2E for signals 18/19 — fixture-blocked) and signal 22 (provenance round-trip via interpolation — Phase 4 territory, currently in flight by a parallel agent).
