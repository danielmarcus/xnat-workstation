...
export default function SegmentationPanel({ sourceImageIds }: SegmentationPanelProps) {
  ...
  const handleSaveLocal = useCallback(async (
    ...
    try {
      ...
      // Same unaccepted-interpolation check as handleUploadXnat. See notes
      // there — autosave silently excludes auto-generated contours, but
      // manual saves should not.
      if (dicomType === 'RTSTRUCT') {
        const unacceptedCount = interpolationAcceptance.countAutoGeneratedForSegmentation(segmentationId);
        if (unacceptedCount > 0) {
          const plural = unacceptedCount === 1 ? '' : 's';
          const confirmed = await showConfirmDialog({
            title: 'Unaccepted Interpolated Contours',
            message:
              `${unacceptedCount} interpolated contour${plural} ${unacceptedCount === 1 ? 'is' : 'are'} not yet accepted and won't be saved. `
              + `Accept all and save?`,
            confirmLabel: 'Accept & Save',
            cancelLabel: 'Cancel',
          });
          if (!confirmed) return;
          interpolationAcceptance.acceptAllForSegmentation(segmentationId);
        }
      }
  ...
  const handleUploadXnat = useCallback(async (
    ...
    // RTSTRUCT only: interpolated contours remain "auto-generated" (dashed,
    // excluded from export) until the user accepts them. Prompt before a
    // manual save so the user can either promote them all, or cancel and
    // review. Auto-save silently excludes them — that's intentional, since
    // autosave is crash-recovery and shouldn't commit provisional work.
    if (dicomType === 'RTSTRUCT') {
      const unacceptedCount = interpolationAcceptance.countAutoGeneratedForSegmentation(segmentationId);
      if (unacceptedCount > 0) {
        const plural = unacceptedCount === 1 ? '' : 's';
