...
import * as autoAcceptInterpolated from './segmentationService/autoAcceptInterpolated';
...
import {
  ...
  isSegmentLockedInternal,
  takeAutoGeneratedBuffer,
} from './segmentationService/historyMemo';
import {
  copySelected as clipboardCopySelected,
  pasteToActiveSlice as clipboardPasteToActiveSlice,
...
export const segmentationService = {
  ...
  initialize(): void {
    ...
    // Wire the contour-clipboard subsystem with the orchestrator-side
    // dependencies it needs. Late-bound to avoid a circular import via the
    // service object (`segmentationService.X` references inside the module
    // would require this file to be fully evaluated before import).
    wireContourClipboard({
      getSegmentationType,
      getSegmentLocked: (segmentationId, segmentIndex) =>
        segmentationService.getSegmentLocked(segmentationId, segmentIndex),
      setActiveSegmentIndex: (segmentationId, segmentIndex) =>
        segmentationService.setActiveSegmentIndex(segmentationId, segmentIndex),
      activateOnViewport: (viewportId, segmentationId) =>
    ...
    // Phase 4.5: feed `containerStoreSync.buildMember` the load-in-progress
    // signal so freshly synthesized members default to provenance
    // 'imported' during a SEG / RTSTRUCT load (per §D7.2).
    containerStoreSync.setLoadInProgressGate(() => isSegLoadInProgress());
    ...
    // module have been retired.
    autoAcceptInterpolated.initialize();

    // Phase 4.1: stamp provenance + auto-marker on members whose contours
    // were just interpolated. Independent of `autoAcceptInterpolated` —
    ...
    provenanceStamping.wireProvenance({
      ...
      clearInterpolatedMark: (memberId) => {
        // Phase 4.4: only flip provenance if the member is currently
        // 'interpolated' — leave 'imported', 'auto-segmented', etc.
        // alone. Always clear the auto-marker since it's a UI-only
        // signal that "there are unreviewed interpolated bits."
        for (const { containerId } of containerBridge.listAll()) {
  ...
  pasteCopiedContourAnnotationToActiveSlice(): boolean {
    return clipboardPasteToActiveSlice();
  },
  ...
  dispose(): void {
    ...
    autoAcceptInterpolated.dispose();
