...
export function useViewport({
  ...
  useEffect(() => {
    ...
    const syncState = (kind: 'init' | 'voi' | 'image' | 'camera'): void => {
      ...
      // Zoom is cheap and changes on camera; always refresh it.
      ...
      if (kind === 'init' || kind === 'image' || kind === 'camera') {
        ...
        // Refetch slice metadata only when the displayed image actually changed.
    ...
    viewportService
      ...
      .then(() => {
        ...
        // slice index / W/L / zoom / metadata are live. The deleted
        // CornerstoneViewport did this; the unified path had dropped it (stale
        // readouts). Handles both stack and volume viewports.
