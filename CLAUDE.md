...
**Keyboard focus is a different thing and keeps its own ring.** Left alone, Chrome paints `:focus-visible` in the **macOS system accent colour** — a colour the app never chose, differing per machine, and appearing only after a keyboard interaction. In the viewport it is suppressed (not tab-reachable, and the active ring already marks it); in the XNAT browser it is **restyled, not removed** (`globals.css`), to a subtle zinc, because a scan row is keyboard-reachable and the ring is the only cue a keyboard user gets. Removing it there would be an accessibility regression.
