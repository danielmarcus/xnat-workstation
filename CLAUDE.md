...
**Keyboard focus never lives in the side panels.** The XNAT browser and the annotations panel are mouse-driven and shortcuts are viewport-only, so a focus ring in either marks something the user cannot navigate to and cannot act on. Both halves are enforced: the outline is suppressed (`globals.css`) and focus stranded by a click is released (`lib/hotkeys/keepFocusInViewports.ts`), so Space and Enter cannot re-activate a panel row instead of reaching the viewport. Text entry is exempt — filter and rename fields keep the keyboard while typing. Creating an annotation does **not** move focus either: the keyboard stays on the viewport and keystrokes are **captured** into the new container's label, then (on Enter) its first member's label. `hotkeyService.setNameCaptureHandler` receives them ahead of the shortcut map; the labels render the draft as TEXT (`capturedDraft` on `ContainerRow`/`MemberRow`), never as an input, so no focus ring can appear on them and the viewport keeps receiving scroll and navigation keys while a name is typed — only printable characters, Backspace, Enter and Escape are consumed. Enter accepts a stage, Escape or a click anywhere else ends the sequence, and whatever was not typed keeps its default name. Two earlier designs were rejected from use: focusing the label stole the keyboard from the image, and opening the editor unfocused meant the label had to be clicked before it could be typed in. Left alone Chrome draws that ring in the **macOS system accent colour**, which is why it looked arbitrary. Making the panels keyboard-navigable was tried and rejected (branch `keyboard-navigation`): focus leaving the viewports is itself the problem.
...
**Multi-viewport coupling**: containers are **viewport-scoped**, and an annotation belongs to **the scan it was drawn on**. It renders on every viewport showing that scan — a second viewport on the same series, or the planes of an MPR — and on no others; those viewports list it, and it carries a cross-panel pill (`↗ 2 panels`) when it is on more than one. Drawing is allowed only on a viewport showing its own scan. The **unsaved indicator deliberately does not scope** — unsaved work in an unfocused viewport must stay visible, since its review dialog is the only way to save it.

> Requirements A2b/A2c (a mask also rendering, dimmed and read-only, on a *sibling* series of the same exam unless a measured anatomical shift suggested the patient had moved) were **withdrawn 2026-09-18** as incorrect, and the code and tests built to them deleted. That rule never reached users — the panel's create path only ever attached a mask to its own series.
